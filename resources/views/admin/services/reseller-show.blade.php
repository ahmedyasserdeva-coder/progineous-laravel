...
@section('title', $service->service_name . ' - ' . ($isArabic ?? false ? 'إدارة استضافة الموزعين' : 'Reseller Hosting Management'))
...
<div class="container mx-auto px-4 py-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3 rtl:space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="text-sm font-medium text-gray-500">{{ __('crm.reseller_hosting') ?? ($isArabic ? 'استضافة الموزعين' : 'Reseller Hosting') }}</span>
                </div>
            </li>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 mb-6 shadow-sm">
        <div class="flex items-center justify-between flex-wrap gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="https://{{ $service->server->hostname }}:2087/login/?user={{ $service->username }}&pass={{ urlencode($service->decrypted_password) }}" target="_blank" 
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        ...
        <!-- Activate -->
        <form action="{{ route('admin.services.update-status', $service) }}" method="POST" class="contents">
            @csrf
            ...
            <button type="submit" class="group flex flex-col items-center gap-2 p-4 bg-white border border-gray-200 rounded-xl hover:border-emerald-300 hover:bg-emerald-50/50 transition-all">
                ...
                <span class="text-xs font-medium text-gray-700 text-center">{{ $isArabic ? 'تفعيل' : 'Activate' }}</span>
            </button>
        </form>
        @endif
                    ...
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-xl">
                            <div>
                                ...
                                <dd class="mt-1 text-sm font-semibold text-gray-900">{{ __('crm.reseller_hosting') ?? ($isArabic ? 'استضافة الموزعين' : 'Reseller Hosting') }}</dd>
                            </div>
                        </div>
                        <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-xl hover:bg-gray-50 transition-colors">
                            <div>
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-xl hover:bg-gray-50 transition-colors">
                            <div>
                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide">{{ $isArabic ? 'حسابات cPanel' : 'cPanel Accounts' }}</dt>
                                <dd class="mt-1 text-sm font-semibold text-gray-900 flex items-center gap-2">
                                    <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                    {{ $cpanelAccounts }} {{ $isArabic ? 'حساب' : 'Accounts' }}
                                </dd>
                            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-slate-100/50">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-base font-semibold text-slate-900">{{ $isArabic ? 'بيانات الدخول لـ WHM' : 'WHM Login Credentials' }}</h3>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <div class="text-center py-6">
                        ...
                        <h4 class="text-lg font-semibold text-gray-900 mb-2">{{ $isArabic ? 'بيانات الدخول غير مكتملة' : 'Credentials Not Set' }}</h4>
                        <p class="text-gray-500 mb-4">{{ $isArabic ? 'يرجى إضافة بيانات الدخول لـ WHM ليتمكن العميل من الوصول لحسابه' : 'Please add WHM login credentials so the client can access their account' }}</p>
                        <button type="button" onclick="document.getElementById('editWhmCredentialsModal').classList.remove('hidden')" 
                            ...
                            {{ $isArabic ? 'إضافة بيانات الدخول' : 'Add Credentials' }}
                        </button>
                    </div>
                    @else
                    <dl class="space-y-4">
                        <div class="p-4 bg-gray-50 rounded-xl">
                            <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">{{ $isArabic ? 'رابط الدخول' : 'Login URL' }}</dt>
                            <dd class="flex items-center gap-2">
                                <code class="flex-1 text-sm font-mono text-slate-700 bg-white px-3 py-2 rounded-lg border border-gray-200 overflow-x-auto">{{ $whmLoginUrl }}</code>
                                <button onclick="navigator.clipboard.writeText('{{ $whmLoginUrl }}')" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Copy">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                </button>
                                <a href="{{ $whmLoginUrl }}" target="_blank" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Open">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="p-4 bg-gray-50 rounded-xl">
                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">{{ $isArabic ? 'اسم المستخدم' : 'Username' }}</dt>
                                <dd class="flex items-center gap-2">
                                    <code class="flex-1 text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg border border-gray-200">{{ $whmUsername }}</code>
                                    <button onclick="navigator.clipboard.writeText('{{ $whmUsername }}')" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Copy">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                    </button>
                                </dd>
                            </div>
                            <div class="p-4 bg-gray-50 rounded-xl" x-data="{ showPassword: false }">
                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">{{ $isArabic ? 'كلمة المرور' : 'Password' }}</dt>
                                <dd class="flex items-center gap-2">
                                    <code class="flex-1 text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg border border-gray-200">
                                        ...
                                        <span x-show="showPassword">{{ $whmPassword }}</span>
                                    </code>
                                    ...
                                    <button onclick="navigator.clipboard.writeText('{{ $whmPassword }}')" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Copy">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                    </button>
                                </dd>
                            </div>
                ...
                <div class="fixed inset-0 flex items-center justify-center">
                    <div class="relative bg-white rounded-lg shadow-xl w-96 mx-4 p-5 z-10" onclick="event.stopPropagation()">
                        ...
                        <form action="{{ route('admin.services.change-password', $service) }}" method="POST" x-data="{ password: '', showPassword: false }">
                            ...
                            <div class="mb-4">
                                <button type="button" @click="password = '{{ \Illuminate\Support\Str::random(16) }}'" class="text-sm text-slate-700 hover:text-slate-900">
                    ...
                    <div class="relative bg-white rounded-lg shadow-xl w-[500px] mx-4 p-5 z-10" onclick="event.stopPropagation()">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                ...
                                {{ $isArabic ? 'تغيير عدد حسابات cPanel' : 'Change cPanel Accounts' }}
                            </h3>
                        ...
                        <form action="{{ route('admin.services.update-cpanel-accounts', $service) }}" method="POST">
                            @csrf
                            @method('PUT')
                            @php
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ $isArabic ? 'العدد الحالي' : 'Current Count' }}
                                </label>
                                <input type="text" value="{{ $currentCpanelAccounts }} {{ $isArabic ? 'حساب' : 'Accounts' }}" readonly
                                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-100 text-gray-600">
                            </div>
                            
                            @if(isset($cpanelTiers) && $cpanelTiers->count() > 0)
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ $isArabic ? 'اختر باقة cPanel' : 'Select cPanel Tier' }} <span class="text-red-500">*</span>
                                </label>
                                <div class="space-y-2">
                                    @if(isset($product) && $product->base_cpanel_accounts)
                                    <label class="flex items-center justify-between p-3 border rounded-lg cursor-pointer hover:bg-blue-50 transition-colors {{ $currentCpanelAccounts == $product->base_cpanel_accounts ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
                                        <div class="flex items-center gap-3">
                                            <input type="radio" name="cpanel_accounts" value="{{ $product->base_cpanel_accounts }}" 
                                                   class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                                                   {{ $currentCpanelAccounts == $product->base_cpanel_accounts ? 'checked' : '' }}>
                                            <div>
                                                <span class="font-medium text-gray-900">{{ $product->base_cpanel_accounts }} {{ $isArabic ? 'حساب' : 'Accounts' }}</span>
                                                <span class="text-xs text-gray-500 {{ $isArabic ? 'mr-2' : 'ml-2' }}">({{ $isArabic ? 'الأساسي' : 'Base' }})</span>
                                            </div>
                                        </div>
                                        <span class="text-sm font-semibold text-emerald-600">{{ $isArabic ? 'مجاني' : 'Free' }}</span>
                                    </label>
                                    @endif
                                    
                                    @foreach($cpanelTiers as $tier)
                                    @php
                                        $tierPrice = $tier->$priceColumn ?? $tier->monthly_price;
                                    @endphp
                                    <label class="flex items-center justify-between p-3 border rounded-lg cursor-pointer hover:bg-blue-50 transition-colors {{ $currentCpanelAccounts == $tier->tier ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
                                        <div class="flex items-center gap-3">
                                            <input type="radio" name="cpanel_accounts" value="{{ $tier->tier }}" 
                                                   class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                                                   {{ $currentCpanelAccounts == $tier->tier ? 'checked' : '' }}>
                                            <div>
                                                <span class="font-medium text-gray-900">{{ $tier->tier }} {{ $isArabic ? 'حساب' : 'Accounts' }}</span>
                                            </div>
                                        </div>
                                        <span class="text-sm font-semibold text-blue-600">+{{ number_format($tierPrice, 2) }} {{ __('crm.currency') }}/{{ __('crm.' . str_replace('-', '_', $billingCycle)) }}</span>
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ $isArabic ? 'العدد الجديد' : 'New Count' }} <span class="text-red-500">*</span>
                                </label>
                                <input type="number" name="cpanel_accounts" required min="1" max="1000"
                                       value="{{ $currentCpanelAccounts }}"
                                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <p class="mt-1 text-xs text-gray-500">{{ $isArabic ? 'أدخل عدد حسابات cPanel المسموح بها للعميل' : 'Enter the number of cPanel accounts allowed for the client' }}</p>
                            </div>
                            @endif
                            ...
                            <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                                <p class="text-xs text-amber-700">
                                    ...
                                    {{ $isArabic ? 'ملاحظة: هذا سيغير العدد في قاعدة البيانات فقط. يجب عليك تحديث حدود الحساب في WHM يدوياً إذا لزم الأمر.' : 'Note: This will only change the count in the database. You may need to update the account limits in WHM manually if required.' }}
                                </p>
                            </div>
                            <div class="flex gap-3">
                    ...
                    <div class="relative bg-white rounded-lg shadow-xl w-[500px] mx-4 p-5 z-10" onclick="event.stopPropagation()">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                ...
                                {{ $isArabic ? 'بيانات الدخول لـ WHM' : 'WHM Login Credentials' }}
                            </h3>
                        ...
                        <form action="{{ route('admin.services.update-whm-credentials', $service) }}" method="POST">
                            @csrf
                            @method('PUT')
                            @php
                                $metadata = $service->metadata ?? [];
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ $isArabic ? 'رابط الدخول WHM' : 'WHM Login URL' }} <span class="text-red-500">*</span>
                                    </label>
                                    <input type="url" name="whm_login_url" required
                                           value="{{ old('whm_login_url', $existingLoginUrl) }}"
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">{{ $isArabic ? 'مثال: https://server.example.com:2087' : 'Example: https://server.example.com:2087' }}</p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ $isArabic ? 'اسم المستخدم' : 'Username' }} <span class="text-red-500">*</span>
                                    </label>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ $isArabic ? 'كلمة المرور' : 'Password' }} <span class="text-red-500">*</span>
                                    </label>
                                    <div class="relative" x-data="{ showPassword: false }">
                                        <input :type="showPassword ? 'text' : 'password'" name="password" required
                                               value="{{ $existingPassword }}"
                                               placeholder="••••••••"
                                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600 {{ $isArabic ? 'pl-10' : 'pr-10' }}">
                                        <button type="button" @click="showPassword = !showPassword" 
                                                class="absolute {{ $isArabic ? 'left-3' : 'right-3' }} top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                            <svg x-show="!showPassword" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                            </svg>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ $isArabic ? 'عنوان IP الحساب' : 'Account IP Address' }}
                                        <span class="text-xs font-normal text-gray-500">{{ $isArabic ? '(اختياري)' : '(optional)' }}</span>
                                    </label>
                                    <input type="text" name="whm_ip_address"
                                           value="{{ old('whm_ip_address', $metadata['whm_ip_address'] ?? '') }}"
                                           placeholder="192.168.1.1"
                                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600">
                                    <p class="mt-1 text-xs text-gray-500">{{ $isArabic ? 'عنوان IP المخصص لحساب الريسلر' : 'The dedicated IP address for this reseller account' }}</p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ $isArabic ? 'ملاحظة للعميل' : 'Note for Customer' }}
                                        <span class="text-xs font-normal text-gray-500">{{ $isArabic ? '(اختياري)' : '(optional)' }}</span>
                                    </label>
                                    <textarea name="whm_customer_note" rows="3"
                                              placeholder="{{ $isArabic ? 'أدخل أي ملاحظات أو تعليمات للعميل...' : 'Enter any notes or instructions for the customer...' }}"
                                              class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600 resize-none">{{ old('whm_customer_note', $metadata['whm_customer_note'] ?? '') }}</textarea>
                                    <p class="mt-1 text-xs text-gray-500">{{ $isArabic ? 'هذه الملاحظة ستظهر للعميل في صفحة الخدمة' : 'This note will be displayed to the customer on their service page' }}</p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <input type="checkbox" name="activate_service" id="activate_service" value="1"
                                           ...
                                           {{ $service->status === 'pending' ? 'checked' : '' }}>
                                    <label for="activate_service" class="text-sm text-gray-700">
                                        {{ $isArabic ? 'تفعيل الخدمة بعد حفظ البيانات' : 'Activate service after saving credentials' }}
                                    </label>
                                </div>
                            </div>
                            <div class="bg-slate-50 border border-slate-200 rounded-lg p-3 mt-4 mb-4">
                                <p class="text-xs text-slate-800">
                                    ...
                                    {{ $isArabic ? 'هذه البيانات ستظهر للعميل في صفحة خدمته. تأكد من صحتها قبل الحفظ.' : 'These credentials will be visible to the client on their service page. Please verify them before saving.' }}
                                </p>
                            </div>
                            <div class="flex gap-3">
                                ...
                                <button type="submit" 
                                    ...
                                    {{ $isArabic ? 'حفظ البيانات' : 'Save Credentials' }}
                                </button>
                            </div>
                        </form>
                    </div>
