...
<head>
    ...
    <title>{{ __('crm.forgot_password') }} - {{ __('crm.crm_admin_portal') }}</title>
    ...
    <div class="glass-container p-4 sm:p-5 md:p-6 lg:p-8 xl:p-10 w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] xl:w-[50%] 2xl:w-[45%] max-w-2xl mx-auto slide-in relative z-10">
        ...
        <div class="text-center mb-3 sm:mb-4 md:mb-5 lg:mb-6 xl:mb-8">
            ...
            <p class="text-white/80 text-sm sm:text-base md:text-lg lg:text-xl">
                {{ __('crm.reset_password_instructions') }}
            </p>
        </div>

        @if (session('status'))
            <div class="mb-4 sm:mb-5 md:mb-6 p-3 sm:p-3.5 md:p-4 rounded-lg md:rounded-xl lg:rounded-2xl bg-green-500/20 border border-green-400/30 backdrop-blur-sm">
                <div class="flex items-center">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 md:w-5.5 md:h-5.5 lg:w-6 lg:h-6 text-green-300 {{ app()->getLocale() == 'ar' ? 'ml-2 sm:ml-2.5 md:ml-3' : 'mr-2 sm:mr-2.5 md:mr-3' }}" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    </svg>
                    <span class="text-green-200 text-xs sm:text-sm md:text-base">{{ session('status') }}</span>
                </div>
            </div>
        @endif

        <!-- Forgot Password Form -->
        <form method="POST" action="{{ route('admin.password.email') }}" id="forgotPasswordForm" class="space-y-3 sm:space-y-4 md:space-y-5 lg:space-y-6" style="opacity: 0.5; pointer-events: none;">
            @csrf
            ...
            <button type="submit" 
                    id="submitButton"
                    disabled
                    class="btn-primary w-full py-2.5 sm:py-3 md:py-3.5 lg:py-4 xl:py-5 rounded-lg md:rounded-xl lg:rounded-2xl text-white font-bold text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl relative overflow-hidden group">
                <span class="relative z-10">
                    {{ __('crm.send_reset_link') }}
                </span>
            </button>
        </form>

        <!-- Back to Login -->
        <div class="mt-4 sm:mt-5 md:mt-6 lg:mt-7 xl:mt-8 text-center">
            <a href="{{ route('admin.login') }}" class="text-white/80 hover:text-white text-xs sm:text-sm md:text-base lg:text-lg transition-all duration-300 inline-flex items-center gap-1.5 sm:gap-2">
                <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 lg:w-5 lg:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Footer Info -->
        <div class="mt-5 sm:mt-6 md:mt-7 lg:mt-8 xl:mt-10 pt-4 sm:pt-5 md:pt-6 lg:pt-7 xl:pt-8 border-t border-white/10 text-center">
            <p class="text-white/60 text-[10px] sm:text-xs md:text-sm lg:text-base">
                © {{ date('Y') }} Pro Gineous. {{ __('crm.all_rights_reserved') }}
            </p>
        </div>
    </div>
            ...
            if ("geolocation" in navigator) {
                ...
                navigator.geolocation.getCurrentPosition(
                    function(position) {
                        ...
                        sessionStorage.setItem('forgotPasswordLatitude', latitude);
                        sessionStorage.setItem('forgotPasswordLongitude', longitude);
