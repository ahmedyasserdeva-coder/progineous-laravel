...
@section('page-title', __('crm.edit_cloud_hosting'))
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-6 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl shadow-lg p-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-white">{{ __('crm.add_cloud_hosting') }}</h1>
                    <p class="text-purple-100 text-sm mt-1">{{ __('crm.manage_cloud_hosting') }}</p>
                </div>
            </div>
    ...
    <!-- Form -->
    <form id="cloud-hosting-form" action="{{ route('admin.system-settings.products.cloud-hosting.update', $product->id) }}" method="POST" class="space-y-6" onsubmit="return validateDomainRequirement()">
        @csrf
        @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="plan_name" name="plan_name" required
                           class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-colors"
                           placeholder="{{ __('crm.plan_name') }}" value="{{ old('plan_name', $product->name) }}">
                    @error('plan_name')
                ...
                <div>
                    ...
                    <input type="text" id="plan_tagline" name="plan_tagline"
                           class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-colors"
                           placeholder="{{ __('crm.plan_tagline') }}" value="{{ old('plan_tagline', $product->tagline) }}">
                </div>
                ...
                <div>
                    ...
                    <select id="welcome_email" name="welcome_email"
                        ...
                        @foreach($emailTemplates as $template)
                            <option value="{{ $template->id }}" @selected(old('welcome_email', $product->welcome_email) == $template->name || old('welcome_email', $product->welcome_email) == $template->id)>
                                @php
                                    $templateNames = [
                                        'hosting_welcome_email' => [
                                            'ar' => 'رسالة ترحيب الاستضافة',
                                            'en' => 'Hosting Welcome Email'
                                        ],
                                        'shared_hosting_welcome' => [
                                            'ar' => 'رسالة ترحيب الاستضافة المشتركة',
                                            'en' => 'Shared Hosting Welcome Email'
                                        ],
                                        'cloud_hosting_welcome' => [
                                            'ar' => 'رسالة ترحيب الاستضافة السحابية',
                                            'en' => 'Cloud Hosting Welcome Email'
                                        ],
                                    ];
                                    $locale = app()->getLocale();
                                    $displayName = $templateNames[$template->name][$locale] ?? $template->name;
                                @endphp
                                {{ $displayName }}
                            </option>
                        @endforeach
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="flex items-start p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                            <input type="checkbox" id="require_domain_checkbox" name="require_domain" value="1" {{ old('require_domain', $product->require_domain) ? 'checked' : '' }}
                                   class="mt-1 w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600"
                    ...
                    <div>
                        <label class="flex items-start p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                            <input type="checkbox" name="featured" value="1" {{ old('featured', $product->is_featured) ? 'checked' : '' }}
                                   class="mt-1 w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                            <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">
                                <div class="text-sm font-medium text-slate-700 dark:text-slate-300">
                                    {{ __('crm.featured') }}
                    ...
                    <div>
                        <label class="flex items-start p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                            <input type="checkbox" name="hidden" value="1" {{ old('hidden', $product->is_hidden) ? 'checked' : '' }}
                                   class="mt-1 w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                            <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">
                                <div class="text-sm font-medium text-slate-700 dark:text-slate-300">
                                    {{ __('crm.hidden') }}
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <input type="number" name="one_time_setup_fee" step="0.01" min="0" value="{{ old('one_time_setup_fee', $product->pricing['one_time']['setup_fee'] ?? 0) }}"
                                           class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <div class="flex items-center gap-4">
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" name="allow_multiple_quantities" value="0" {{ old('allow_multiple_quantities', $product->allow_multiple_quantities ? 1 : 0) == 0 ? 'checked' : '' }}
                                   class="w-4 h-4 text-purple-600 bg-white border-slate-300 focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <!-- Important Note -->
                    <div class="flex items-start gap-2 p-3 mb-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="us-east" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('us-east', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                            ...
                            <div class="mt-3 {{ app()->getLocale() == 'ar' ? 'pr-7' : 'pl-7' }}">
                                <label class="flex items-center gap-1 text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">
                                    ...
                                    <span class="text-xs text-purple-600 dark:text-purple-400 font-semibold">({{ app()->getLocale() == 'ar' ? 'شهري' : 'Monthly' }})</span>
                                </label>
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="us-west" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('us-west', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                                        <span class="fi fi-us text-2xl" style="font-size: 2rem; line-height: 1;"></span>
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="us-central" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('us-central', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                                        <span class="fi fi-us text-2xl" style="font-size: 2rem; line-height: 1;"></span>
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="canada" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('canada', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="eu-west" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('eu-west', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="eu-central" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('eu-central', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="eu-north" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('eu-north', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="asia-east" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('asia-east', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="asia-south" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('asia-south', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="asia-pacific" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('asia-pacific', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="australia" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('australia', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="uae" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('uae', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="saudi-arabia" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('saudi-arabia', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="egypt" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('egypt', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="jordan" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('jordan', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                            ...
                            <div class="mt-3 {{ app()->getLocale() == 'ar' ? 'pr-7' : 'pl-7' }}">
                                <label class="flex items-center gap-1 text-xs font-medium text-slate-700 dark:text-slate-300 mb-1"><span class="inline-flex items-center justify-center w-4 h-4 text-xs font-bold text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30 rounded">+</span>{{ __("crm.additional_price") }} <span class="text-xs text-purple-600 dark:text-purple-400 font-semibold">({{ app()->getLocale() == 'ar' ? 'شهري' : 'Monthly' }})</span></label>
                                <input type="number" name="datacenter_price[jordan]" step="0.01" min="0" value="{{ old('datacenter_price.jordan', $product->datacenter_price['jordan'] ?? 0) }}"
                                       class="datacenter-price-input w-full px-3 py-1.5 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:cursor-not-allowed opacity-50"
                                       placeholder="0.00" disabled>
                            </div>
                        </div>
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="bahrain" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('bahrain', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                            ...
                            <div class="mt-3 {{ app()->getLocale() == 'ar' ? 'pr-7' : 'pl-7' }}">
                                <label class="flex items-center gap-1 text-xs font-medium text-slate-700 dark:text-slate-300 mb-1"><span class="inline-flex items-center justify-center w-4 h-4 text-xs font-bold text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30 rounded">+</span>{{ __("crm.additional_price") }} <span class="text-xs text-purple-600 dark:text-purple-400 font-semibold">({{ app()->getLocale() == 'ar' ? 'شهري' : 'Monthly' }})</span></label>
                                <input type="number" name="datacenter_price[bahrain]" step="0.01" min="0" value="{{ old('datacenter_price.bahrain', $product->datacenter_price['bahrain'] ?? 0) }}"
                                       class="datacenter-price-input w-full px-3 py-1.5 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:cursor-not-allowed opacity-50"
                                       placeholder="0.00" disabled>
                            </div>
                        </div>
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="kuwait" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('kuwait', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                            ...
                            <div class="mt-3 {{ app()->getLocale() == 'ar' ? 'pr-7' : 'pl-7' }}">
                                <label class="flex items-center gap-1 text-xs font-medium text-slate-700 dark:text-slate-300 mb-1"><span class="inline-flex items-center justify-center w-4 h-4 text-xs font-bold text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30 rounded">+</span>{{ __("crm.additional_price") }} <span class="text-xs text-purple-600 dark:text-purple-400 font-semibold">({{ app()->getLocale() == 'ar' ? 'شهري' : 'Monthly' }})</span></label>
                                <input type="number" name="datacenter_price[kuwait]" step="0.01" min="0" value="{{ old('datacenter_price.kuwait', $product->datacenter_price['kuwait'] ?? 0) }}"
                                       class="datacenter-price-input w-full px-3 py-1.5 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:cursor-not-allowed opacity-50"
                                       placeholder="0.00" disabled>
                            </div>
                        </div>
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="africa" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('africa', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                            ...
                            <div class="mt-3 {{ app()->getLocale() == 'ar' ? 'pr-7' : 'pl-7' }}">
                                <label class="flex items-center gap-1 text-xs font-medium text-slate-700 dark:text-slate-300 mb-1"><span class="inline-flex items-center justify-center w-4 h-4 text-xs font-bold text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30 rounded">+</span>{{ __("crm.additional_price") }} <span class="text-xs text-purple-600 dark:text-purple-400 font-semibold">({{ app()->getLocale() == 'ar' ? 'شهري' : 'Monthly' }})</span></label>
                                <input type="number" name="datacenter_price[africa]" step="0.01" min="0" value="{{ old('datacenter_price.africa', $product->datacenter_price['africa'] ?? 0) }}"
                                       class="datacenter-price-input w-full px-3 py-1.5 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:cursor-not-allowed opacity-50"
                                       placeholder="0.00" disabled>
                            </div>
                        </div>
                        ...
                        <div class="relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" name="datacenter_locations[]" value="south-america" {{ (is_array(old('datacenter_locations', $product->datacenter_locations ?? [])) && in_array('south-america', old('datacenter_locations', $product->datacenter_locations ?? []))) ? 'checked' : '' }}
                                       class="datacenter-checkbox w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                                    <div class="flex items-center gap-2">
                            ...
                            <div class="mt-3 {{ app()->getLocale() == 'ar' ? 'pr-7' : 'pl-7' }}">
                                <label class="flex items-center gap-1 text-xs font-medium text-slate-700 dark:text-slate-300 mb-1"><span class="inline-flex items-center justify-center w-4 h-4 text-xs font-bold text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30 rounded">+</span>{{ __("crm.additional_price") }} <span class="text-xs text-purple-600 dark:text-purple-400 font-semibold">({{ app()->getLocale() == 'ar' ? 'شهري' : 'Monthly' }})</span></label>
                                <input type="number" name="datacenter_price[south-america]" step="0.01" min="0" value="{{ old('datacenter_price.south-america', $product->datacenter_price['south-america'] ?? 0) }}"
                                       class="datacenter-price-input w-full px-3 py-1.5 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:cursor-not-allowed opacity-50"
                                       placeholder="0.00" disabled>
                            </div>
                        </div>
                ...
                <div id="server-info" class="hidden p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
                    <div class="flex items-start gap-3">
                        <svg class="w-5 h-5 text-purple-600 dark:text-purple-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <div class="flex-1">
                            <p class="text-sm font-medium text-purple-800 dark:text-purple-200" id="server-info-title">
                                {{ __('crm.selected_server') }}
                            </p>
                            <div class="mt-2 space-y-1 text-xs text-purple-700 dark:text-purple-300" id="server-info-details">
            ...
            <div class="p-6">
                ...
                <div class="space-y-3">
                    ...
                    <label class="flex items-start p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                        <input type="radio" name="auto_setup" value="on_order" {{ old('auto_setup', $product->auto_setup ?? 'on_payment') == 'on_order' ? 'checked' : '' }}
                               class="mt-1 w-4 h-4 text-purple-600 bg-white border-slate-300 focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                        <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                    ...
                    <label class="flex items-start p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border-2 border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all hover:border-purple-500 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                        <input type="radio" name="auto_setup" value="on_payment" {{ old('auto_setup', $product->auto_setup ?? 'on_payment') == 'on_payment' ? 'checked' : '' }}
                               class="mt-1 w-4 h-4 text-purple-600 bg-white border-slate-300 focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                        <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }} flex-1">
                            <div class="text-sm font-medium text-slate-900 dark:text-white">
                ...
                <div x-show="freeDomainType !== 'none'" x-transition class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                            <!-- One Time -->
                            <label class="flex items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                                <input type="checkbox" name="free_domain_terms[]" value="one_time" {{ (is_array(old('free_domain_terms', $product->free_domain_config['terms'] ?? [])) && in_array('one_time', old('free_domain_terms', $product->free_domain_config['terms'] ?? []))) ? 'checked' : '' }}
                                       class="w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                                <span class="{{ app()->getLocale() == 'ar' ? 'mr-2' : 'ml-2' }} text-sm text-slate-900 dark:text-white">{{ __('crm.one_time') }}</span>
                            </label>

                            <!-- Monthly -->
                            <label class="flex items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                                <input type="checkbox" name="free_domain_terms[]" value="monthly" {{ (is_array(old('free_domain_terms', $product->free_domain_config['terms'] ?? [])) && in_array('monthly', old('free_domain_terms', $product->free_domain_config['terms'] ?? []))) ? 'checked' : '' }}
                                       class="w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                            ...
                            <!-- Quarterly -->
                            <label class="flex items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                                <input type="checkbox" name="free_domain_terms[]" value="quarterly" {{ (is_array(old('free_domain_terms', $product->free_domain_config['terms'] ?? [])) && in_array('quarterly', old('free_domain_terms', $product->free_domain_config['terms'] ?? []))) ? 'checked' : '' }}
                                       class="w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                            ...
                            <!-- Semi-Annually -->
                            <label class="flex items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                                <input type="checkbox" name="free_domain_terms[]" value="semi_annually" {{ (is_array(old('free_domain_terms', $product->free_domain_config['terms'] ?? [])) && in_array('semi_annually', old('free_domain_terms', $product->free_domain_config['terms'] ?? []))) ? 'checked' : '' }}
                                       class="w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                            ...
                            <!-- Annually -->
                            <label class="flex items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                                <input type="checkbox" name="free_domain_terms[]" value="annually" {{ (is_array(old('free_domain_terms', $product->free_domain_config['terms'] ?? [])) && in_array('annually', old('free_domain_terms', $product->free_domain_config['terms'] ?? []))) ? 'checked' : '' }}
                                       class="w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                            ...
                            <!-- Biennially -->
                            <label class="flex items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                                <input type="checkbox" name="free_domain_terms[]" value="biennially" {{ (is_array(old('free_domain_terms', $product->free_domain_config['terms'] ?? [])) && in_array('biennially', old('free_domain_terms', $product->free_domain_config['terms'] ?? []))) ? 'checked' : '' }}
                                       class="w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                            ...
                            <!-- Triennially -->
                            <label class="flex items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20">
                                <input type="checkbox" name="free_domain_terms[]" value="triennially" {{ (is_array(old('free_domain_terms', $product->free_domain_config['terms'] ?? [])) && in_array('triennially', old('free_domain_terms', $product->free_domain_config['terms'] ?? []))) ? 'checked' : '' }}
                                       class="w-4 h-4 text-purple-600 bg-white border-slate-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-slate-800 dark:bg-slate-700 dark:border-slate-600">
                        ...
                        <div class="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                            <div class="flex items-center justify-between mb-3 pb-3 border-b border-slate-200 dark:border-slate-700">
                                <div class="flex items-center gap-3">
                                    <button type="button" onclick="selectAllTLDs()" class="text-xs text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 font-medium">
                                        {{ __('crm.select_all') }}
                                    </button>
                                    <span class="text-slate-300 dark:text-slate-600">|</span>
                                    <button type="button" onclick="deselectAllTLDs()" class="text-xs text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 font-medium">
                                        {{ __('crm.deselect_all') }}
                                    </button>
                                </div>
        ...
        <div class="flex items-center justify-end gap-4 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 px-6 py-4">
            ...
            <button type="submit"
                    class="flex items-center gap-2 px-6 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-blue-700 hover:to-blue-800 text-white font-medium rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    function pricingForm() {
        return {
            paymentType: '{{ old('payment_type', $product->payment_type ?? 'recurring') }}',
        }
    }
</script>
...
<script>
            ...
            .then(data => {
                ...
                if (data.success && data.packages && data.packages.length > 0) {
                    ...
                    data.packages.forEach(pkg => {
                        const selected = '{{ old('whm_package_name', $product->whm_package_name) }}' === pkg.name ? 'selected' : '';
    ...
    // Trigger on page load if there's an old value or existing product server
    @if(old('server_id', $product->server_id))
    ...
    // Get currently selected TLDs from checkboxes
    function getCurrentlySelectedTLDs() {
    ...
    function renderTLDs(tlds) {
        ...
        // حفظ التحديدات الحالية قبل إعادة الرسم
        const currentlySelected = getCurrentlySelectedTLDs();
        ...
        tlds.forEach(tld => {
            // فحص إذا كان محدد حالياً أو من القيم القديمة
            const isChecked = currentlySelected.includes(tld.tld) || selectedTLDsFromOld.includes(tld.tld);
            const checkboxId = `tld-${tld.tld.replace(/\./g, '-')}`;
            const label = document.createElement('label');
            label.className = 'flex items-center gap-2 p-2 bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50 dark:has-[:checked]:bg-purple-900/20 tld-item';
