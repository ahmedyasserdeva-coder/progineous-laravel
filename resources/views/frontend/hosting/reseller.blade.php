...
@section('title', __('frontend.reseller_hosting') . ' - ' . config('app.name'))

@section('meta_description', __('frontend.reseller_hosting_meta_desc') ?? 'استضافة ريسلر قوية ومرنة مع WHM/cPanel، موارد غير محدودة، أسعار تنافسية، وإدارة سهلة. ابدأ عملك كمزود استضافة مع Pro Gineous.')

@section('meta_keywords', __('frontend.reseller_hosting_keywords') ?? 'استضافة ريسلر, reseller hosting, WHM, cPanel, استضافة بيضاء, white label hosting, بيع استضافة, Pro Gineous')

@push('meta')
    <!-- Open Graph / Facebook -->
    <meta property="og:type" content="website">
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:title" content="{{ __('frontend.reseller_hosting') }} - {{ config('app.name') }}">
    <meta property="og:description" content="{{ __('frontend.reseller_hosting_meta_desc') ?? 'استضافة ريسلر قوية ومرنة مع WHM/cPanel وموارد غير محدودة' }}">
    <meta property="og:image" content="{{ asset('assets/reseller-hosting.png') }}">

    <!-- Twitter -->
    <meta property="twitter:card" content="summary_large_image">
    <meta property="twitter:url" content="{{ url()->current() }}">
    <meta property="twitter:title" content="{{ __('frontend.reseller_hosting') }} - {{ config('app.name') }}">
    <meta property="twitter:description" content="{{ __('frontend.reseller_hosting_meta_desc') ?? 'استضافة ريسلر قوية ومرنة' }}">
    <meta property="twitter:image" content="{{ asset('assets/reseller-hosting.png') }}">
...
<section class="relative pt-24 pb-16 lg:pt-32 lg:pb-24 overflow-hidden min-h-[90vh] flex items-center">
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
        <div class="absolute top-20 {{ app()->getLocale() == 'ar' ? 'left-10' : 'right-10' }} w-72 h-72 bg-gradient-to-br from-purple-400/30 to-pink-500/30 rounded-full blur-3xl animate-blob"></div>
        <div class="absolute bottom-20 {{ app()->getLocale() == 'ar' ? 'right-10' : 'left-10' }} w-96 h-96 bg-gradient-to-tr from-orange-400/30 to-red-500/30 rounded-full blur-3xl animate-blob animation-delay-2000"></div>
        ...
        <div class="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div class="text-center lg:text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} space-y-8">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4" data-aos="fade-up" data-aos-delay="200">
                    <a href="#plans" class="group relative inline-flex items-center px-8 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold text-lg rounded-2xl shadow-2xl hover:shadow-purple-500/50 transform hover:scale-105 transition-all duration-300 overflow-hidden">
                        ...
                        <span class="relative z-10">{{ __('frontend.view_plans') ?? 'اطلع على الباقات' }}</span>
                        <svg class="w-5 h-5 {{ app()->getLocale() == 'ar' ? 'mr-2' : 'ml-2' }} relative z-10 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex flex-wrap items-center justify-center lg:justify-start gap-6 pt-8" data-aos="fade-up" data-aos-delay="300">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-gray-700">{{ app()->getLocale() == 'ar' ? 'WHM/cPanel مجاني' : 'Free WHM/cPanel' }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-6 h-6 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
                        </svg>
                        <span class="text-sm font-semibold text-gray-700">{{ app()->getLocale() == 'ar' ? 'شهادات SSL مجانية' : 'Free SSL Certificates' }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-6 h-6 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
                        </svg>
                        <span class="text-sm font-semibold text-gray-700">{{ app()->getLocale() == 'ar' ? 'نسخ احتياطي يومي' : 'Daily Backups' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="relative hidden lg:block" data-aos="fade-left" data-aos-delay="300">
                <div class="relative">
                    ...
                    <!-- Floating Elements -->
                    <div class="absolute -top-8 -{{ app()->getLocale() == 'ar' ? 'left' : 'right' }}-8 bg-white p-4 rounded-2xl shadow-2xl animate-float">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-xs text-gray-500 font-medium">{{ app()->getLocale() == 'ar' ? 'أرباحك الشهرية' : 'Your Monthly Profit' }}</p>
                                <p class="text-lg font-bold text-gray-900">$2,500+</p>
                            </div>
                        </div>
                    </div>

                    <div class="absolute bottom-8 -{{ app()->getLocale() == 'ar' ? 'right' : 'left' }}-8 bg-white p-4 rounded-2xl shadow-2xl animate-float" style="animation-delay: 1s;">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-xs text-gray-500 font-medium">{{ app()->getLocale() == 'ar' ? 'عملائك النشطين' : 'Active Clients' }}</p>
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16 lg:mb-20">
            <div class="inline-flex items-center px-4 py-2 rounded-full bg-purple-100 border border-purple-200 mb-6">
                <svg class="w-4 h-4 {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }} text-purple-600" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z"></path>
                    <path fill-rule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clip-rule="evenodd"></path>
                </svg>
                <span class="text-sm font-bold text-purple-700">{{ __('frontend.features') ?? 'المميزات' }}</span>
            </div>

            <h2 class="text-3xl sm:text-4xl lg:text-5xl font-black text-gray-900 mb-6">
                {{ __('frontend.reseller_hosting_features') ?? 'مميزات استضافة الريسلر' }}
            </h2>
            <p class="text-lg lg:text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
                {{ __('frontend.reseller_hosting_features_desc') ?? 'كل ما تحتاجه لبدء وإدارة عملك في مجال الاستضافة بنجاح' }}
            </p>
        </div>
            ...
            <div class="group relative" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="relative h-full p-8 bg-white rounded-3xl border-2 border-gray-100 hover:border-purple-200 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                    ...
                    <h3 class="text-xl lg:text-2xl font-bold text-gray-900 mb-3 group-hover:text-purple-600 transition-colors">
                        {{ app()->getLocale() == 'ar' ? 'White Label كامل' : 'Full White Label' }}
                    </h3>
            ...
            <div class="group relative" data-aos="fade-up" data-aos-delay="200">
                ...
                <div class="relative h-full p-8 bg-white rounded-3xl border-2 border-gray-100 hover:border-pink-200 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                    ...
                    <h3 class="text-xl lg:text-2xl font-bold text-gray-900 mb-3 group-hover:text-pink-600 transition-colors">
                        {{ app()->getLocale() == 'ar' ? 'موارد غير محدودة' : 'Unlimited Resources' }}
                    </h3>
                    <p class="text-gray-600 leading-relaxed">
                        {{ app()->getLocale() == 'ar' 
            ...
            <div class="group relative" data-aos="fade-up" data-aos-delay="300">
                ...
                <div class="relative h-full p-8 bg-white rounded-3xl border-2 border-gray-100 hover:border-orange-200 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                    ...
                    <h3 class="text-xl lg:text-2xl font-bold text-gray-900 mb-3 group-hover:text-orange-600 transition-colors">
                        {{ app()->getLocale() == 'ar' ? 'أسعار مرنة' : 'Flexible Pricing' }}
                    </h3>
                    <p class="text-gray-600 leading-relaxed">
                        {{ app()->getLocale() == 'ar' 
            ...
            <div class="group relative" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="relative h-full p-8 bg-white rounded-3xl border-2 border-gray-100 hover:border-blue-200 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                    ...
                    <h3 class="text-xl lg:text-2xl font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors">
                        {{ app()->getLocale() == 'ar' ? 'دعم فني مجاني' : 'Free Technical Support' }}
                    </h3>
                    <p class="text-gray-600 leading-relaxed">
                        {{ app()->getLocale() == 'ar' 
            ...
            <div class="group relative" data-aos="fade-up" data-aos-delay="200">
                ...
                <div class="relative h-full p-8 bg-white rounded-3xl border-2 border-gray-100 hover:border-green-200 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                    ...
                    <h3 class="text-xl lg:text-2xl font-bold text-gray-900 mb-3 group-hover:text-green-600 transition-colors">
                        {{ app()->getLocale() == 'ar' ? 'SSL مجاني مدى الحياة' : 'Free SSL Certificates' }}
                    </h3>
                    <p class="text-gray-600 leading-relaxed">
                        {{ app()->getLocale() == 'ar' 
            ...
            <div class="group relative" data-aos="fade-up" data-aos-delay="300">
                ...
                <div class="relative h-full p-8 bg-white rounded-3xl border-2 border-gray-100 hover:border-indigo-200 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                    ...
                    <h3 class="text-xl lg:text-2xl font-bold text-gray-900 mb-3 group-hover:text-indigo-600 transition-colors">
                        {{ app()->getLocale() == 'ar' ? 'نسخ احتياطي تلقائي' : 'Automatic Backups' }}
                    </h3>
                    <p class="text-gray-600 leading-relaxed">
                        {{ app()->getLocale() == 'ar' 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                {{ __('frontend.why_choose_progenious') ?? 'لماذا تختار Pro Gineous لاستضافة الموزعين؟' }}
            </h2>
            <p class="text-lg text-gray-600 max-w-3xl mx-auto">
                {{ __('frontend.why_choose_progenious_desc') ?? 'نقدم لك أفضل الحلول لبدء وتنمية عملك في الاستضافة مع دعم متكامل وتقنيات متطورة' }}
            </p>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-8 hover:shadow-xl transition-all duration-300" data-aos="fade-up" data-aos-delay="100">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-3">
                    {{ __('frontend.guaranteed_uptime') ?? 'وقت تشغيل مضمون 99.9%' }}
                </h3>
                <p class="text-gray-600 leading-relaxed">
                    {{ __('frontend.guaranteed_uptime_desc') ?? 'بنية تحتية موثوقة مع مراقبة على مدار الساعة لضمان استمرارية خدمات عملائك دون انقطاع' }}
                </p>
            </div>
            ...
            <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-8 hover:shadow-xl transition-all duration-300" data-aos="fade-up" data-aos-delay="200">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-3">
                    {{ __('frontend.instant_activation') ?? 'تفعيل فوري' }}
                </h3>
                <p class="text-gray-600 leading-relaxed">
                    {{ __('frontend.instant_activation_desc') ?? 'احصل على حساب الريسلر الخاص بك جاهزاً للاستخدام فوراً بعد الدفع دون أي تأخير' }}
                </p>
            </div>
            ...
            <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-8 hover:shadow-xl transition-all duration-300" data-aos="fade-up" data-aos-delay="300">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-3">
                    {{ __('frontend.competitive_pricing') ?? 'أسعار تنافسية' }}
                </h3>
                <p class="text-gray-600 leading-relaxed">
                    {{ __('frontend.competitive_pricing_desc') ?? 'احصل على أفضل قيمة مقابل استثمارك مع أسعار مرنة تتيح لك تحقيق أرباح جيدة' }}
                </p>
            </div>
            ...
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl p-8 hover:shadow-xl transition-all duration-300" data-aos="fade-up" data-aos-delay="400">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-3">
                    {{ __('frontend.expert_support') ?? 'دعم فني متخصص' }}
                </h3>
                <p class="text-gray-600 leading-relaxed">
                    {{ __('frontend.expert_support_desc') ?? 'فريق دعم خبير متاح على مدار الساعة لمساعدتك أنت وعملائك في أي وقت' }}
                </p>
            </div>
            ...
            <div class="bg-gradient-to-br from-red-50 to-rose-50 rounded-2xl p-8 hover:shadow-xl transition-all duration-300" data-aos="fade-up" data-aos-delay="500">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-3">
                    {{ __('frontend.scalable_solutions') ?? 'حلول قابلة للتوسع' }}
                </h3>
                <p class="text-gray-600 leading-relaxed">
                    {{ __('frontend.scalable_solutions_desc') ?? 'نمو عملك معنا سهل وسلس، يمكنك الترقية إلى باقة أعلى في أي وقت دون توقف' }}
                </p>
            </div>
            ...
            <div class="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-8 hover:shadow-xl transition-all duration-300" data-aos="fade-up" data-aos-delay="600">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-3">
                    {{ __('frontend.detailed_documentation') ?? 'دليل شامل' }}
                </h3>
                <p class="text-gray-600 leading-relaxed">
                    {{ __('frontend.detailed_documentation_desc') ?? 'وثائق تقنية شاملة وفيديوهات تعليمية لمساعدتك في بدء وإدارة عملك بكفاءة' }}
                </p>
            </div>
        </div>
        ...
        <div class="mt-16 bg-gradient-to-r from-purple-600 to-pink-600 rounded-3xl p-8 md:p-12" data-aos="fade-up">
            <div class="grid md:grid-cols-4 gap-8 text-center text-white">
                <div>
                    ...
                    <div class="text-purple-100">{{ __('frontend.uptime_guarantee') ?? 'ضمان وقت التشغيل' }}</div>
                </div>
                <div>
                    <div class="text-4xl font-bold mb-2">24/7</div>
                    <div class="text-purple-100">{{ __('frontend.technical_support') ?? 'دعم فني' }}</div>
                </div>
                <div>
                    <div class="text-4xl font-bold mb-2">∞</div>
                    <div class="text-purple-100">{{ __('frontend.unlimited_accounts') ?? 'حسابات غير محدودة' }}</div>
                </div>
                <div>
                    <div class="text-4xl font-bold mb-2">100%</div>
                    <div class="text-purple-100">{{ __('frontend.white_label') ?? 'White Label' }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
            ...
            <p class="text-lg text-gray-600 max-w-2xl mx-auto">
                {{ __('frontend.reseller_plans_desc') ?? 'باقات ريسلر مرنة تناسب جميع الاحتياجات وجميع الميزانيات' }}
            </p>
        </div>
            ...
            <div x-data="{ cycle: 'monthly', open: false }">
                ...
                <div class="flex justify-center mb-12">
                    <div class="relative inline-block w-full max-w-md">
                        <button @click="open = !open" type="button"
                            class="relative w-full cursor-pointer rounded-2xl bg-white py-4 px-6 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} shadow-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 border-2 border-gray-100">
                            <span class="flex items-center justify-between">
                                <span class="flex items-center">
                                    <span class="block text-base font-semibold text-gray-900" x-text="
                                        ...
                                        '{{ __('frontend.triennially') ?? 'ثلاث سنوات' }}'
                                    "></span>
                                </span>
                        ...
                        <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95"
                            ...
                            style="display: none;">
                            <div class="py-2">
                                <button @click="cycle = 'monthly'; open = false" class="block w-full px-6 py-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} text-sm text-gray-700 hover:bg-gray-100 transition-colors">
                                    {{ __('frontend.monthly') ?? 'شهري' }}
                                </button>
                                <button @click="cycle = 'quarterly'; open = false" class="block w-full px-6 py-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} text-sm text-gray-700 hover:bg-gray-100 transition-colors">
                                    {{ __('frontend.quarterly') ?? 'ربع سنوي' }} <span class="text-green-600 font-semibold">({{ __('frontend.save_10') ?? 'وفر 10%' }})</span>
                                </button>
                                <button @click="cycle = 'semiannually'; open = false" class="block w-full px-6 py-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} text-sm text-gray-700 hover:bg-gray-100 transition-colors">
                                    {{ __('frontend.semiannually') ?? 'نصف سنوي' }} <span class="text-green-600 font-semibold">({{ __('frontend.save_15') ?? 'وفر 15%' }})</span>
                                </button>
                                <button @click="cycle = 'annually'; open = false" class="block w-full px-6 py-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} text-sm text-gray-700 hover:bg-gray-100 transition-colors">
                                    {{ __('frontend.annually') ?? 'سنوي' }} <span class="text-orange-600 font-semibold">({{ __('frontend.save_20') ?? 'وفر 20%' }})</span>
                                </button>
                                <button @click="cycle = 'biennially'; open = false" class="block w-full px-6 py-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} text-sm text-gray-700 hover:bg-gray-100 transition-colors">
                                    {{ __('frontend.biennially') ?? 'سنتين' }} <span class="text-orange-600 font-semibold">({{ __('frontend.save_30') ?? 'وفر 30%' }})</span>
                                </button>
                                <button @click="cycle = 'triennially'; open = false" class="block w-full px-6 py-3 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} text-sm text-gray-700 hover:bg-gray-100 transition-colors">
                                    {{ __('frontend.triennially') ?? 'ثلاث سنوات' }} <span class="text-red-600 font-semibold">({{ __('frontend.save_40') ?? 'وفر 40%' }})</span>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                @foreach($plans as $index => $plan)
                    ...
                    <div class="relative group" data-aos="fade-up" data-aos-delay="{{ ($index + 1) * 100 }}" x-data="{ 
                        ...
                        @if($plan->is_featured)
                            ...
                            <div class="absolute -top-4 left-1/2 transform -translate-x-1/2 z-20">
                                <div class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-bold rounded-full shadow-xl">
                                    <svg class="w-4 h-4 {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }} animate-pulse" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div class="relative h-full bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border-2 {{ $plan->is_featured ? 'border-purple-500 scale-105' : 'border-gray-200' }} overflow-hidden">
                            @if($plan->is_featured)
                            ...
                            <div class="relative p-8">
                                ...
                                <div class="mb-6">
                                    <div class="flex items-baseline gap-2">
                                        ...
                                        <span class="text-gray-500 font-medium" x-text="
                                            cycle === 'monthly' ? '/{{ __('frontend.per_month') ?? 'شهر' }}' :
                                            cycle === 'quarterly' ? '/{{ __('frontend.per_3_months') ?? '3 أشهر' }}' :
                                            cycle === 'semiannually' ? '/{{ __('frontend.per_6_months') ?? '6 أشهر' }}' :
                                            cycle === 'annually' ? '/{{ __('frontend.per_year') ?? 'سنة' }}' :
                                            cycle === 'biennially' ? '/{{ __('frontend.per_2_years') ?? 'سنتين' }}' :
                                            '/{{ __('frontend.per_3_years') ?? '3 سنوات' }}'
                                        "></span>
                                    </div>
                                </div>

                                <!-- CTA Button -->
                                <a href="{{ route('products.show', $plan->id) }}" class="block w-full text-center px-6 py-4 {{ $plan->is_featured ? 'bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700' : 'bg-gray-900 hover:bg-gray-800' }} text-white font-bold rounded-xl transition-all duration-300 transform hover:scale-105 mb-8">
                                ...
                                @if($showFreeDomain && count($freeDomainTlds) > 0)
                                        ...
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div class="flex-1">
                                                <h5 class="text-sm font-bold text-green-900 mb-2">
                                                    {{ __('frontend.free_domain_included') ?? 'نطاق مجاني متضمن' }}
                                                </h5>
                                                <p class="text-xs text-green-700 mb-3">
                                                    {{ __('frontend.free_domain_desc') ?? 'احصل على نطاق مجاني عند الاشتراك السنوي أو الأطول' }}
                                                </p>
                                                <div class="flex flex-wrap gap-2">
                                                    @foreach($freeDomainTlds as $tld)
                                                ...
                                                <p class="text-xs text-green-600 mt-2 font-medium">
                                                    @if(isset($freeDomainConfig['type']) && $freeDomainConfig['type'] === 'reg_transfer_renewal')
                                                        {{ __('frontend.free_domain_note_renewal') ?? '* النطاق مجاني للتسجيل والنقل والتجديد' }}
                                                    @else
                                                        {{ __('frontend.free_domain_note') ?? '* النطاق مجاني للسنة الأولى فقط' }}
                                                    @endif
                                                </p>
                                            </div>
                                        </div>
                                ...
                                @if($features && count($features) > 0)
                                    <div class="space-y-4">
                                        <h4 class="text-sm font-bold text-gray-900 uppercase tracking-wide mb-4">{{ __('frontend.features_included') ?? 'المميزات المتضمنة' }}</h4>
                                        <ul class="space-y-3">
        ...
        @else
            <div class="text-center py-20">
                ...
                <p class="text-gray-600 text-lg">
                    {{ __('frontend.reseller_plans_coming_soon') ?? 'باقات استضافة الريسلر ستكون متاحة قريباً' }}
                </p>
            </div>
        @endif
    </div>
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-8" data-aos="fade-up" data-aos-delay="100">
                <div class="flex items-center mb-6">
                    <div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl flex items-center justify-center {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                        </svg>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-900">
                        {{ __('frontend.great_business_features') ?? 'مميزات تجارية رائعة' }}
                    </h3>
                </div>
                <ul class="space-y-4">
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-blue-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.completely_whitelabel') ?? 'White Label بالكامل' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-blue-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.instant_setup') ?? 'إعداد فوري' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-blue-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.unlimited_free_migrations') ?? 'انتقالات مجانية غير محدودة' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-blue-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.choice_of_locations') ?? 'اختيار من 20 موقع' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-blue-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.offsite_backups') ?? 'نسخ احتياطية خارجية لمدة 30 يوم' }}</span>
                    </li>
                </ul>
            </div>
            ...
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-8" data-aos="fade-up" data-aos-delay="200">
                <div class="flex items-center mb-6">
                    <div class="w-12 h-12 bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl flex items-center justify-center {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z"></path>
                        </svg>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-900">
                        {{ __('frontend.key_technical_benefits') ?? 'المميزات التقنية الرئيسية' }}
                    </h3>
                </div>
                <ul class="space-y-4">
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-purple-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.unlimited_nvme_ssd') ?? 'مساحة NvME/SSD غير محدودة' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-purple-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.distributed_anycast_dns') ?? 'DNS Anycast موزع مع خوادم أسماء مخصصة' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-purple-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.high_cloudlinux_lve') ?? 'حدود CloudLinux LVE عالية' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-purple-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.free_ssl_certificates') ?? 'شهادات SSL مجانية' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-purple-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.unlimited_bandwidth') ?? 'باندويث غير محدود' }}</span>
                    </li>
                </ul>
            </div>
            ...
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl p-8" data-aos="fade-up" data-aos-delay="300">
                <div class="flex items-center mb-6">
                    <div class="w-12 h-12 bg-gradient-to-br from-orange-500 to-amber-500 rounded-xl flex items-center justify-center {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"></path>
                        </svg>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-900">
                        {{ __('frontend.additional_features') ?? 'مميزات إضافية' }}
                    </h3>
                </div>
                <ul class="space-y-4">
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-orange-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.free_domain_registration') ?? 'تسجيل أو نقل دومين مجاني واحد' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-orange-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.email_reputation_protection') ?? 'حماية سمعة البريد الإلكتروني Mailchannels/SpamXperts' }}</span>
                    </li>
                    <li class="flex items-start">
                        <svg class="w-6 h-6 text-orange-600 flex-shrink-0 mt-0.5 {{ app()->getLocale() == 'ar' ? 'ml-4' : 'mr-4' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span class="text-gray-700">{{ __('frontend.premium_tools') ?? 'LiteSpeed، CloudLinux، LScache، Jetbackup، Imunify360، Softaculous، Sitepad والمزيد...' }}</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="mt-12 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 rounded-3xl p-8 md:p-12 text-center text-white" data-aos="fade-up" data-aos-delay="400">
            <div class="max-w-4xl mx-auto">
                <h3 class="text-2xl md:text-3xl font-bold mb-4">
                    {{ __('frontend.world_leading_infrastructure') ?? 'بنية تحتية رائدة عالمياً' }}
                </h3>
                <p class="text-lg text-white/90 leading-relaxed">
                    {{ __('frontend.world_leading_infrastructure_desc') ?? 'كل هذا مدعوم بالبنية التحتية الرائدة عالمياً من Pro Gineous، مما يضمن أداءً عالياً واستقراراً لا مثيل له لعملك وعملائك.' }}
                </p>
            </div>
        </div>
    </div>
...
<section class="py-20 bg-gradient-to-br from-gray-50 to-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                {{ __('frontend.cpanel_reseller_features') ?? 'مميزات استضافة الموزعين cPanel' }}
            </h2>
            <p class="text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
                {{ __('frontend.cpanel_reseller_features_desc') ?? 'بيئة الاستضافة المثالية: قوية ومرنة، سهلة الاستخدام ومع إضافات مهمة مثل النسخ الاحتياطية، شهادات SSL وخوادم الأسماء المخصصة متضمنة كمعيار.' }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16" data-aos="fade-up" data-aos-delay="100">
            <div class="bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-purple-100 hover:border-purple-300">
                ...
                <h4 class="font-bold text-gray-900">{{ __('frontend.whm_cpanel') ?? 'WHM/cPanel' }}</h4>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-blue-100 hover:border-blue-300">
                ...
                <h4 class="font-bold text-gray-900">{{ __('frontend.software') ?? 'البرمجيات' }}</h4>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-green-100 hover:border-green-300">
                ...
                <h4 class="font-bold text-gray-900">{{ __('frontend.whitelabel') ?? 'Whitelabel' }}</h4>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-orange-100 hover:border-orange-300">
                ...
                <h4 class="font-bold text-gray-900">{{ __('frontend.support_24_7') ?? 'دعم 24/7' }}</h4>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-indigo-100 hover:border-indigo-300">
                ...
                <h4 class="font-bold text-gray-900">{{ __('frontend.server_setup') ?? 'إعداد الخادم' }}</h4>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-pink-100 hover:border-pink-300">
                ...
                <h4 class="font-bold text-gray-900">{{ __('frontend.email') ?? 'البريد الإلكتروني' }}</h4>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-teal-100 hover:border-teal-300">
                ...
                <h4 class="font-bold text-gray-900">{{ __('frontend.fully_managed') ?? 'إدارة كاملة' }}</h4>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-red-100 hover:border-red-300">
                ...
                <h4 class="font-bold text-gray-900">{{ __('frontend.instant_setup') ?? 'إعداد فوري' }}</h4>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-2">
                            {{ __('frontend.fully_managed') ?? 'إدارة كاملة' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.fully_managed_desc') ?? 'لا يُشترط خبرة تقنية. نحن نتولى إدارة الخادم بالكامل، مما يضمن تشغيل استضافتك بسرعة وأمان.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="200">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-2">
                            {{ __('frontend.cpanel_whm') ?? 'cPanel/WHM' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.cpanel_whm_desc') ?? 'واجهة WHM الرائدة في السوق، والتي تتيح لك إنشاء حسابات cPanel فردية لكل موقع من مواقعك.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="300">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-2">
                            {{ __('frontend.everything_included') ?? 'كل شيء متضمن' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.everything_included_desc') ?? 'لا توجد رسوم خفية أو بيع إضافي للميزات الأساسية. شهادات SSL، الانتقالات، حسابات البريد الإلكتروني، خوادم الأسماء المخصصة - كل هذا وأكثر متضمن في السعر الشهري.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="400">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-2">
                            {{ __('frontend.free_migrations') ?? 'انتقالات مجانية' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.free_migrations_desc') ?? 'يمكننا نقل مواقعك من مزودي الخدمة الخارجيين إلينا، بالسرعة والوقت الذي تختاره. إذا كنت تريد، يمكننا البدء فوراً.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="500">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-2">
                            {{ __('frontend.money_back_guarantee') ?? 'ضمان استرداد الأموال' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.money_back_guarantee_desc') ?? 'جرّب لمدة 30 يوماً. سنرد لك أموالك إذا لم تكن راضياً. الموزعون المميزون لديهم ضمان استرداد لمدة 45 يوماً.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="600">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-2">
                            {{ __('frontend.instant_setup') ?? 'إعداد فوري' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.instant_setup_desc') ?? 'اطلب الآن وكن جاهزاً للعمل في ثوانٍ. إذا كانت لديك عمليات انتقال، يمكننا البدء بها على الفور أيضاً.' }}
                        </p>
                    </div>
                </div>
            </div>
...
<section class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                {{ __('frontend.whm_title') ?? 'Web Host Manager (WHM)' }}
            </h2>
            <p class="text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
                {{ __('frontend.whm_desc') ?? 'Web Host Manager هو واجهة مستعرض رسومية تتيح لك إدارة حسابات الاستضافة الخاصة بك بسرعة وسهولة. إنشاء حساب بسيط مثل إدخال اسم نطاق، ويمكنك بعد ذلك تسجيل الدخول إلى لوحة التحكم الخاصة بكل موقع بنقرة واحدة.' }}
            </p>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-8 mb-12">
                ...
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-2xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.bulk_cpanel_management') ?? 'إدارة cPanel الجماعية' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.bulk_cpanel_management_desc') ?? 'قم بإدراج جميع حسابات cPanel، وتعليقها وإنهائها. قم بتسجيل الدخول إلى كل واحد منها بسهولة، دون الحاجة إلى تذكر بيانات تسجيل دخول منفصلة.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="200">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-2xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.easy_migrations') ?? 'انتقالات سهلة' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.easy_migrations_desc') ?? 'إذا كنت تستخدم بالفعل WHM أو cPanel مع مضيفك الحالي، يمكننا بسهولة ترحيل النسخ الاحتياطية لـ cPanel إلى حساب الموزع الجديد الخاص بك.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="300">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-2xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.packages_feature_lists') ?? 'الباقات وقوائم المميزات' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.packages_feature_lists_desc') ?? 'قم بإنشاء باقات الاستضافة الخاصة بك، مع حدود مساحة القرص والباندويث والموارد الأخرى. قم بترقية وتخفيض المواقع بين الباقات، ومراجعة استخدامها.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="400">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-2xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.backup_tools') ?? 'أدوات النسخ الاحتياطي' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.backup_tools_desc') ?? 'يتم عمل نسخة احتياطية لكل موقع مرتين يومياً، ويمكنك الاستعادة الذاتية باستخدام Jetbackup. نمنحك أيضاً وصولاً كاملاً إلى أداة النسخ الاحتياطي الخاصة بـ cPanel. الموزعون المميزون يحصلون على نسخ احتياطية أربع مرات يومياً.' }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-3xl p-8 md:p-12 text-white" data-aos="fade-up" data-aos-delay="500">
            <div class="grid md:grid-cols-2 gap-8 items-center">
                <div>
                    <h3 class="text-2xl md:text-3xl font-bold mb-4">
                        {{ __('frontend.whm_power_control') ?? 'قوة وتحكم كامل' }}
                    </h3>
                    <p class="text-lg text-white/90 leading-relaxed mb-6">
                        {{ __('frontend.whm_power_control_desc') ?? 'WHM يمنحك السيطرة الكاملة على جميع حسابات الاستضافة الخاصة بك من مكان واحد. إدارة احترافية مع أدوات قوية.' }}
                    </p>
                    <div class="flex flex-wrap gap-4">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-white {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                            <span>{{ __('frontend.one_click_login') ?? 'تسجيل دخول بنقرة واحدة' }}</span>
                        </div>
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-white {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                            <span>{{ __('frontend.resource_monitoring') ?? 'مراقبة الموارد' }}</span>
                        </div>
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-white {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                            <span>{{ __('frontend.automated_backups') ?? 'نسخ احتياطية تلقائية' }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
                        <div class="space-y-4">
                            <div class="flex items-center justify-between">
                                <span class="text-white/80">{{ __('frontend.daily_backups') ?? 'نسخ احتياطية يومية' }}</span>
                                <span class="text-2xl font-bold">2x</span>
                            </div>
                            <div class="h-px bg-white/20"></div>
                            <div class="flex items-center justify-between">
                                <span class="text-white/80">{{ __('frontend.premium_backups') ?? 'نسخ احتياطية مميزة' }}</span>
                                <span class="text-2xl font-bold">4x</span>
                            </div>
                            <div class="h-px bg-white/20"></div>
                            <div class="flex items-center justify-between">
                                <span class="text-white/80">{{ __('frontend.jetbackup_access') ?? 'وصول Jetbackup' }}</span>
...
<section class="py-20 bg-gradient-to-br from-gray-50 to-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                {{ __('frontend.software_tools') ?? 'البرمجيات والأدوات' }}
            </h2>
            <p class="text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
                {{ __('frontend.software_tools_desc') ?? 'أدوات قوية ومرنة لتثبيت وإدارة التطبيقات بسهولة. كل ما تحتاجه لبناء مواقع ويب احترافية.' }}
            </p>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-blue-100 hover:border-blue-300" data-aos="fade-up" data-aos-delay="100">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.one_click_wordpress') ?? 'WordPress بنقرة واحدة' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.one_click_wordpress_desc') ?? 'تثبيت فوري لـ WordPress من خلال Softaculous. كن جاهزاً للعمل في بضع نقرات فقط.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-purple-100 hover:border-purple-300" data-aos="fade-up" data-aos-delay="200">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.optimised') ?? 'محسّن' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.optimised_desc') ?? 'نحن نعلم البرامج التي ستقوم بتشغيلها ونحن نحسنها بالفعل. حدود عالية، إعدادات منطقية.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-green-100 hover:border-green-300" data-aos="fade-up" data-aos-delay="300">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.software_autoinstalls') ?? '300+ تطبيق بتثبيت تلقائي' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.software_autoinstalls_desc') ?? 'اختر من بين أكثر من 300 تطبيق يمكنك تثبيته بنقرة واحدة، بدون أي رسوم إضافية.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-orange-100 hover:border-orange-300" data-aos="fade-up" data-aos-delay="400">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.install_old_versions') ?? 'تثبيت النسخ القديمة' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.install_old_versions_desc') ?? 'تصفح البرامج حسب الفئة، ثم قم بتثبيت أحدث إصدار مستقر، أو الإصدارات التاريخية السابقة.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-indigo-100 hover:border-indigo-300" data-aos="fade-up" data-aos-delay="500">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.php_configuration') ?? 'إعدادات PHP' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.php_configuration_desc') ?? 'قم بتغيير إصدار PHP وضبط إعدادات PHP مباشرة من داخل cPanel.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-teal-100 hover:border-teal-300" data-aos="fade-up" data-aos-delay="600">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.ssh_enabled') ?? 'SSH مُفعّل' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.ssh_enabled_desc') ?? 'SSH مُفعّل افتراضياً على كل حساب cPanel تقوم بإنشائه (المنفذ 22). لا حاجة لتعديل الإعدادات أو طلب الإذن.' }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-12 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-3xl p-8 md:p-12 text-white text-center" data-aos="fade-up" data-aos-delay="700">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
                <div>
                    ...
                    <div class="text-blue-100 text-sm md:text-base">{{ __('frontend.applications') ?? 'تطبيق' }}</div>
                </div>
                <div>
                    <div class="text-4xl md:text-5xl font-bold mb-2">1-Click</div>
                    <div class="text-blue-100 text-sm md:text-base">{{ __('frontend.installation') ?? 'تثبيت' }}</div>
                </div>
                <div>
                    <div class="text-4xl md:text-5xl font-bold mb-2">SSH</div>
                    <div class="text-blue-100 text-sm md:text-base">{{ __('frontend.access') ?? 'وصول' }}</div>
                </div>
                <div>
                    <div class="text-4xl md:text-5xl font-bold mb-2">PHP</div>
                    <div class="text-blue-100 text-sm md:text-base">{{ __('frontend.customization') ?? 'تخصيص' }}</div>
                </div>
            </div>
        </div>
    </div>
...
<section class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                {{ __('frontend.advanced_features') ?? 'المميزات المتقدمة' }}
            </h2>
            <p class="text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
                {{ __('frontend.advanced_features_desc') ?? 'مميزات احترافية متقدمة لبناء علامتك التجارية وإدارة عملك بكفاءة وسهولة.' }}
            </p>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="100">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.custom_nameservers') ?? 'خوادم أسماء مخصصة' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.custom_nameservers_desc') ?? 'خوادم الأسماء المخصصة هي خوادم DNS التي يتم تعيينها على اسم النطاق، على سبيل المثال ns1.yourhostingbrand.com و ns2.yourhostingbrand.com. قم بإعداد خوادم الأسماء المخصصة بدون تكلفة إضافية.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="200">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.fully_whitelabel') ?? 'Whitelabel بالكامل' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.fully_whitelabel_desc') ?? 'ابنِ وأدِر عملك الخاص في استضافة الويب، مع هويتك التجارية الخاصة. منصة الموزعين الخاصة بنا مُعلّمة بالكامل، لذلك لن يعرف عملاؤك أبداً عن Pro Gineous.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="300">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.anycast_dns') ?? 'Anycast DNS' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.anycast_dns_desc') ?? 'نقوم بعمل Anycast لعناوين IP لخوادم DNS الخاصة بنا، مما يعني أنها "معلنة" من مواقع متعددة حول العالم. يتم توجيه الطلبات بعد ذلك بناءً على أقصر مسار إلى أقرب عقدة قادرة على خدمة الطلب.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="400">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.single_dns_cluster') ?? 'مجموعة DNS واحدة' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.single_dns_cluster_desc') ?? 'أتمتة حسابات موزعين متعددة، دون الحاجة إلى إنشاء مجموعات متعددة من سجلات DNS. بغض النظر عن عدد الحسابات التي لديك معنا، يمكنك استخدام مجموعة واحدة من خوادم الأسماء.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="500">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.billing_integration') ?? 'تكامل الفوترة' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.billing_integration_desc') ?? 'منصتنا متوافقة مع برامج الفوترة مثل Upmind، WHMCS، Blesta، Hostbill وبرامج أخرى سهلة الاستخدام.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-red-50 to-pink-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="600">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.lve_controls') ?? 'ضوابط LVE' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.lve_controls_desc') ?? 'كموزع، يمكنك تعيين حدود LVE الخاصة بك للباقات داخل حسابك. نحن نحدد الموارد لكل حساب cPanel، ولكن إذا كنت ترغب يمكنك تقليل هذه الحدود على أساس كل حساب، على سبيل المثال إذا كنت تريد بيع باقات أصغر وأكبر.' }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-12 bg-gradient-to-r from-purple-600 via-pink-600 to-red-600 rounded-3xl p-8 md:p-12 text-white" data-aos="fade-up" data-aos-delay="700">
            <div class="text-center">
                <h3 class="text-2xl md:text-3xl font-bold mb-4">
                    {{ __('frontend.professional_platform') ?? 'منصة احترافية متكاملة' }}
                </h3>
                <p class="text-lg text-white/90 leading-relaxed mb-6 max-w-3xl mx-auto">
                    {{ __('frontend.professional_platform_desc') ?? 'كل ما تحتاجه لبناء عمل استضافة ناجح، من خوادم الأسماء المخصصة إلى تكامل الفوترة الكامل.' }}
                </p>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mt-8">
                    <div class="text-center">
                        <div class="text-3xl font-bold mb-1">100%</div>
                        <div class="text-sm text-white/80">{{ __('frontend.white_label') ?? 'White Label' }}</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-bold mb-1">{{ __('frontend.global') ?? 'Global' }}</div>
                        <div class="text-sm text-white/80">{{ __('frontend.anycast') ?? 'Anycast' }}</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-bold mb-1">{{ __('frontend.custom') ?? 'Custom' }}</div>
                        <div class="text-sm text-white/80">{{ __('frontend.nameservers') ?? 'Nameservers' }}</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-bold mb-1">{{ __('frontend.easy') ?? 'Easy' }}</div>
                        <div class="text-sm text-white/80">{{ __('frontend.integration') ?? 'Integration' }}</div>
                    </div>
                </div>
            </div>
        </div>
...
<section class="py-20 bg-gradient-to-br from-gray-50 to-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                {{ __('frontend.infrastructure_technology') ?? 'البنية التحتية والتكنولوجيا' }}
            </h2>
            <p class="text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
                {{ __('frontend.infrastructure_technology_desc') ?? 'أحدث التقنيات والبنية التحتية القوية لضمان أداء استثنائي وأمان عالي لمواقعك ومواقع عملائك.' }}
            </p>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-blue-100 hover:border-blue-300" data-aos="fade-up" data-aos-delay="100">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.cloudlinux_os') ?? 'CloudLinux OS' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.cloudlinux_os_desc') ?? 'CloudLinux هو تفرع من نظام التشغيل Linux الذي يضيف الأمان ويضع قيوداً على الموارد التي يمكن لأي مستخدم cPanel استخدامها. نحن لا نستخدمه للحد من قوتك - ولكن لحماية المواقع الأخرى في بيئة مشتركة من موقع آخر قد يكون ضاراً أو معطلاً.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-purple-100 hover:border-purple-300" data-aos="fade-up" data-aos-delay="200">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.nvme_ssd') ?? 'NvME / SSD' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.nvme_ssd_desc') ?? 'نقدم NvME حيثما أمكن، وعلى الأقل محركات الأقراص ذات الحالة الصلبة. بسبب البنية التحتية السحابية التي تدعم حسابات الموزعين، يتم حل مشاكل الأجهزة بسهولة مع عدم حدوث أي انقطاع أو حد أدنى من الانقطاع.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-green-100 hover:border-green-300" data-aos="fade-up" data-aos-delay="300">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.imunify360') ?? 'Imunify360' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.imunify360_desc') ?? 'يوفر Imunify360 أمان البرامج الضارة والحماية ضد العديد من تهديدات المواقع الشائعة. يمكنك استخدامه لفحص موقع وتنظيف الملفات المصابة.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-orange-100 hover:border-orange-300" data-aos="fade-up" data-aos-delay="400">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.offsite_backups_title') ?? 'نسخ احتياطية خارجية' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.offsite_backups_desc') ?? 'نأخذ نسختين احتياطيتين يومياً ونخزنهما لمدة 30 يوماً. يمكنك التصفح والتنزيل والاستعادة الذاتية للنسخ الاحتياطية من خلال واجهة Jetbackup في كل من WHM و cPanel.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-indigo-100 hover:border-indigo-300" data-aos="fade-up" data-aos-delay="500">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.replicated_setup') ?? 'إعداد متماثل' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.replicated_setup_desc') ?? 'يتم إنشاء كل حساب موزع وخادم نقوم بتوفيره بشكل متطابق. بمجرد أن يكون لديك موقع واحد معنا، فإنك تعلم أن بيئة الاستضافة لأي موقع آخر معنا ستكون متطابقة تماماً.' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-teal-100 hover:border-teal-300" data-aos="fade-up" data-aos-delay="600">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-5' : 'ml-5' }}">
                        <h3 class="text-xl font-bold text-gray-900 mb-3">
                            {{ __('frontend.infinitely_scalable') ?? 'قابل للتوسع بلا حدود' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed">
                            {{ __('frontend.infinitely_scalable_desc') ?? 'يمكننا إضافة الموارد مثل قوة المعالجة الإضافية أو الذاكرة أو مساحة القرص SSD بسهولة. هذا يعني أننا لا نحتاج إلى تحديد أشياء مثل الباندويث أو التخزين، لأننا يمكننا الاستمرار في زيادة الحدود.' }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-12 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 rounded-3xl p-8 md:p-12 text-white" data-aos="fade-up" data-aos-delay="700">
            <div class="text-center mb-8">
                <h3 class="text-2xl md:text-3xl font-bold mb-3">
                    {{ __('frontend.enterprise_infrastructure') ?? 'بنية تحتية على مستوى المؤسسات' }}
                </h3>
                <p class="text-lg text-white/90 max-w-3xl mx-auto">
                    {{ __('frontend.enterprise_infrastructure_desc') ?? 'تقنيات متقدمة وبنية تحتية قوية لضمان الأداء والأمان الأمثل.' }}
                </p>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
                <div class="text-center">
                    <div class="text-4xl font-bold mb-2">{{ __('frontend.nvme') ?? 'NvME' }}</div>
                    <div class="text-sm text-white/80">{{ __('frontend.ultra_fast_storage') ?? 'تخزين فائق السرعة' }}</div>
                </div>
                <div class="text-center">
                    <div class="text-4xl font-bold mb-2">2x/Day</div>
                    <div class="text-sm text-white/80">{{ __('frontend.backups') ?? 'نسخ احتياطية' }}</div>
                </div>
                <div class="text-center">
                    <div class="text-4xl font-bold mb-2">30 Days</div>
                    <div class="text-sm text-white/80">{{ __('frontend.retention') ?? 'الاحتفاظ' }}</div>
                </div>
                <div class="text-center">
                    <div class="text-4xl font-bold mb-2">∞</div>
                    <div class="text-sm text-white/80">{{ __('frontend.scalable') ?? 'قابل للتوسع' }}</div>
                </div>
            </div>
        </div>
    </div>
...
<section class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                {{ __('frontend.email_features') ?? 'مميزات البريد الإلكتروني' }}
            </h2>
            <p class="text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
                {{ __('frontend.email_features_desc') ?? 'حلول بريد إلكتروني متقدمة لضمان وصول رسائلك بنجاح مع حماية كاملة من البريد المزعج.' }}
            </p>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-10 shadow-xl hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="100">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-6' : 'ml-6' }}">
                        <h3 class="text-2xl font-bold text-gray-900 mb-4">
                            {{ __('frontend.outbound_filtering') ?? 'تصفية البريد الصادر' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed mb-6">
                            {{ __('frontend.outbound_filtering_desc') ?? 'لا حاجة للمعاناة بسبب سمعة البريد السيئة. يتم توجيه جميع البريد الصادر الذي نرسله من خلال أحد شركائنا في تسليم البريد، MailChannels و SpamExperts. هذا يعني أننا يمكننا ضمان إمكانية تسليم بريدك دون أي مشاكل في القائمة السوداء.' }}
                        </p>
                        <div class="flex flex-wrap gap-3">
                            <div class="flex items-center bg-white px-4 py-2 rounded-lg shadow-sm">
                                <svg class="w-5 h-5 text-blue-600 {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                                <span class="text-sm font-medium text-gray-700">{{ __('frontend.mailchannels') ?? 'MailChannels' }}</span>
                            </div>
                            <div class="flex items-center bg-white px-4 py-2 rounded-lg shadow-sm">
                                <svg class="w-5 h-5 text-blue-600 {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                                <span class="text-sm font-medium text-gray-700">{{ __('frontend.spamexperts') ?? 'SpamExperts' }}</span>
                            </div>
                            <div class="flex items-center bg-white px-4 py-2 rounded-lg shadow-sm">
                                <svg class="w-5 h-5 text-blue-600 {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                                <span class="text-sm font-medium text-gray-700">{{ __('frontend.no_blacklisting') ?? 'لا قوائم سوداء' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-10 shadow-xl hover:shadow-2xl transition-all duration-300" data-aos="fade-up" data-aos-delay="200">
                <div class="flex items-start mb-6">
                    ...
                    <div class="{{ app()->getLocale() == 'ar' ? 'mr-6' : 'ml-6' }}">
                        <h3 class="text-2xl font-bold text-gray-900 mb-4">
                            {{ __('frontend.generous_mailing_limits') ?? 'حدود بريدية سخية' }}
                        </h3>
                        <p class="text-gray-600 leading-relaxed mb-6">
                            {{ __('frontend.generous_mailing_limits_desc') ?? 'يمكنك إرسال 100 بريد إلكتروني في الساعة من كل حساب cPanel. يتم فحص كل رسالة، وأي رسائل مصنفة كبريد مزعج سيتم عزلها. يمكننا زيادة هذا الحد ضمن حدود معقولة أيضاً، على الرغم من أننا لا ندعم البريد الجماعي.' }}
                        </p>
                        <div class="space-y-3">
                            <div class="flex items-center justify-between bg-white px-4 py-3 rounded-lg shadow-sm">
                                <span class="text-sm font-medium text-gray-700">{{ __('frontend.emails_per_hour') ?? 'رسائل في الساعة' }}</span>
                            ...
                            <div class="flex items-center bg-white px-4 py-3 rounded-lg shadow-sm">
                                <svg class="w-5 h-5 text-purple-600 {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <span class="text-sm font-medium text-gray-700">{{ __('frontend.spam_scanning') ?? 'فحص البريد المزعج' }}</span>
                            </div>
                            <div class="flex items-center bg-white px-4 py-3 rounded-lg shadow-sm">
                                <svg class="w-5 h-5 text-purple-600 {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <span class="text-sm font-medium text-gray-700">{{ __('frontend.increase_available') ?? 'زيادة متاحة عند الطلب' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-12 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-3xl p-8 md:p-12 text-white text-center" data-aos="fade-up" data-aos-delay="300">
            <h3 class="text-2xl md:text-3xl font-bold mb-3">
                {{ __('frontend.reliable_email_delivery') ?? 'تسليم بريد إلكتروني موثوق' }}
            </h3>
            <p class="text-lg text-white/90 mb-8 max-w-3xl mx-auto">
                {{ __('frontend.reliable_email_delivery_desc') ?? 'شراكات قوية مع أفضل مزودي خدمات البريد لضمان وصول رسائلك دائماً.' }}
            </p>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
                <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                    ...
                    <div class="text-sm text-white/80">{{ __('frontend.per_account') ?? 'لكل حساب' }}</div>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                    <div class="text-4xl font-bold mb-2">100%</div>
                    <div class="text-sm text-white/80">{{ __('frontend.spam_protected') ?? 'محمي من السبام' }}</div>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 md:col-span-1 col-span-2">
                    ...
                    <div class="text-sm text-white/80">{{ __('frontend.blacklist_issues') ?? 'مشاكل القوائم السوداء' }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div data-aos="fade-up">
                <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                    {{ __('frontend.manage_clients_billing') ?? 'إدارة عملائك، الفواتير ودعم العملاء' }}
                </h2>
                <p class="text-lg text-gray-700 mb-6 leading-relaxed">
                    {{ __('frontend.upmind_integration_intro') ?? 'تعمل استضافة الموزعين من Pro Gineous بشكل مثالي مع برنامج إدارة العملاء والفواتير Upmind.' }}
                </p>
                <p class="text-gray-600 mb-6 leading-relaxed">
                    {{ __('frontend.upmind_automation_desc') ?? 'يمكن إنشاء حسابات cPanel لعملائك تلقائياً، وتعليقها، وترقيتها والمزيد مع نظام التكامل العميق الكامل هذا. يمكنك حتى إقران حسابات موزعين متعددة من Pro Gineous!' }}
                </p>
                <p class="text-gray-600 mb-8 leading-relaxed">
                    {{ __('frontend.upmind_setup_desc') ?? 'ببساطة أدخل بيانات اعتماد الموزع الخاصة بك في لوحة Upmind الخاصة بك لبدء أتمتة طلبات عملائك، والمدفوعات ومعالجة الدعم.' }}
                </p>
                ...
                <div class="space-y-4 mb-8">
                    <div class="flex items-start">
                        ...
                        <p class="text-gray-700 {{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                            {{ __('frontend.auto_cpanel_creation') ?? 'إنشاء حسابات cPanel تلقائياً' }}
                        </p>
                    </div>
                    <div class="flex items-start">
                        <div class="w-6 h-6 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-lg flex items-center justify-center flex-shrink-0 mt-1">
                        ...
                        <p class="text-gray-700 {{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                            {{ __('frontend.auto_suspend_upgrade') ?? 'تعليق وترقية تلقائية' }}
                        </p>
                    </div>
                    <div class="flex items-start">
                        <div class="w-6 h-6 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-lg flex items-center justify-center flex-shrink-0 mt-1">
                        ...
                        <p class="text-gray-700 {{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                            {{ __('frontend.multiple_accounts_support') ?? 'دعم حسابات موزعين متعددة' }}
                        </p>
                    </div>
                    <div class="flex items-start">
                        <div class="w-6 h-6 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-lg flex items-center justify-center flex-shrink-0 mt-1">
                        ...
                        <p class="text-gray-700 {{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">
                            {{ __('frontend.free_upmind') ?? 'استخدام Upmind مجاناً' }}
                        </p>
                    </div>
                </div>
                ...
                <a href="#" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl hover:from-blue-700 hover:to-indigo-700 transform hover:scale-105 transition-all duration-300 shadow-lg">
                    {{ __('frontend.read_upmind_guide') ?? 'اقرأ دليل Upmind' }}
            ...
            <div class="relative" data-aos="fade-up" data-aos-delay="100">
                    ...
                    <div class="bg-white rounded-2xl p-8 shadow-lg">
                        <div class="flex items-center justify-between mb-6">
                            <div class="flex items-center">
                                ...
                                <h3 class="text-xl font-bold text-gray-900 {{ app()->getLocale() == 'ar' ? 'mr-4' : 'ml-4' }}">{{ __('frontend.upmind') ?? 'Upmind' }}</h3>
                            </div>
                            <span class="px-3 py-1 bg-green-100 text-green-700 text-sm font-semibold rounded-full">
                                {{ __('frontend.connected') ?? 'متصل' }}
                            </span>
                        </div>
                        
                        <div class="space-y-4">
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                <span class="text-gray-700">{{ __('frontend.active_clients') ?? 'عملاء نشطون' }}</span>
                                <span class="text-2xl font-bold text-blue-600">247</span>
                            </div>
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                <span class="text-gray-700">{{ __('frontend.monthly_revenue') ?? 'إيرادات شهرية' }}</span>
                                <span class="text-2xl font-bold text-green-600">$12,450</span>
                            </div>
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                <span class="text-gray-700">{{ __('frontend.auto_renewals') ?? 'تجديدات تلقائية' }}</span>
                ...
                <div class="absolute -top-4 -right-4 bg-white px-6 py-3 rounded-full shadow-xl border-4 border-blue-100">
                    <p class="text-sm font-bold text-blue-600">{{ __('frontend.free_to_use') ?? 'مجاني' }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div class="relative order-2 lg:order-1" data-aos="fade-up">
                    ...
                    <div class="bg-white rounded-2xl p-8">
                        ...
                        <div class="mb-6">
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-sm font-semibold text-gray-700">{{ __('frontend.migration_progress') ?? 'تقدم الانتقال' }}</span>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center p-3 bg-green-50 rounded-lg border-2 border-green-200">
                                ...
                                <span class="text-sm text-gray-700 {{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">{{ __('frontend.accounts_analyzed') ?? 'تحليل الحسابات' }}</span>
                            </div>
                            <div class="flex items-center p-3 bg-green-50 rounded-lg border-2 border-green-200">
                                <div class="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center flex-shrink-0">
                                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <span class="text-sm text-gray-700 {{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">{{ __('frontend.data_transfer') ?? 'نقل البيانات' }}</span>
                            </div>
                            <div class="flex items-center p-3 bg-blue-50 rounded-lg border-2 border-blue-200 animate-pulse">
                                ...
                                <span class="text-sm text-gray-700 {{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">{{ __('frontend.dns_configuration') ?? 'تكوين DNS' }}</span>
                            </div>
                            <div class="flex items-center p-3 bg-gray-100 rounded-lg border-2 border-gray-200">
                                ...
                                <span class="text-sm text-gray-500 {{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">{{ __('frontend.final_verification') ?? 'التحقق النهائي' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="absolute -top-4 -left-4 bg-white px-6 py-3 rounded-full shadow-xl border-4 border-green-100">
                    <p class="text-sm font-bold text-green-600">{{ __('frontend.free_migration') ?? 'انتقال مجاني' }}</p>
                </div>
                <div class="absolute -bottom-4 -right-4 bg-white px-6 py-3 rounded-full shadow-xl border-4 border-teal-100">
                    <p class="text-sm font-bold text-teal-600">{{ __('frontend.expert_support') ?? 'دعم خبراء' }}</p>
                </div>
            </div>
            ...
            <div class="order-1 lg:order-2" data-aos="fade-up" data-aos-delay="100">
                <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                    {{ __('frontend.moving_easy') ?? 'الانتقال إلى Pro Gineous سهل!' }}
                </h2>
                <p class="text-lg text-gray-700 mb-6 leading-relaxed">
                    {{ __('frontend.migration_intro') ?? 'دعنا نقوم بترحيل مواقعك الإلكترونية وحسابات الاستضافة الموجودة إلى موزع Pro Gineous الخاص بك مجاناً.' }}
                </p>
                <p class="text-gray-600 mb-6 leading-relaxed">
                    {{ __('frontend.migration_details') ?? 'هل لديك حالياً حساب موزع في مكان آخر؟ أو ربما الكثير من حسابات cPanel الفردية؟ يمكن لفريق الهجرة الخبير لدينا نقل استضافتك الحالية إلى حساب الموزع الخاص بك في Pro Gineous.' }}
                </p>
                <p class="text-gray-600 mb-8 leading-relaxed">
                    {{ __('frontend.no_extra_charge') ?? 'لا توجد رسوم إضافية، ويمكنك طلب عمليات انتقال جديدة متى أردت.' }}
                </p>
                ...
                <div class="grid md:grid-cols-2 gap-4 mb-8">
                    <div class="flex items-start p-4 bg-green-50 rounded-xl border border-green-200">
                        ...
                        <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">
                            <h4 class="font-bold text-gray-900 mb-1">{{ __('frontend.completely_free') ?? 'مجاني تماماً' }}</h4>
                            <p class="text-sm text-gray-600">{{ __('frontend.no_hidden_costs') ?? 'بدون تكاليف خفية' }}</p>
                        </div>
                    </div>
                    <div class="flex items-start p-4 bg-green-50 rounded-xl border border-green-200">
                        <svg class="w-6 h-6 text-green-600 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">
                            <h4 class="font-bold text-gray-900 mb-1">{{ __('frontend.unlimited_requests') ?? 'طلبات غير محدودة' }}</h4>
                            <p class="text-sm text-gray-600">{{ __('frontend.migrate_anytime') ?? 'انتقل متى تريد' }}</p>
                        </div>
                    </div>
                    <div class="flex items-start p-4 bg-green-50 rounded-xl border border-green-200">
                        <svg class="w-6 h-6 text-green-600 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">
                            <h4 class="font-bold text-gray-900 mb-1">{{ __('frontend.expert_team') ?? 'فريق خبراء' }}</h4>
                            <p class="text-sm text-gray-600">{{ __('frontend.professional_migration') ?? 'انتقال احترافي' }}</p>
                        </div>
                    </div>
                    <div class="flex items-start p-4 bg-green-50 rounded-xl border border-green-200">
                        <svg class="w-6 h-6 text-green-600 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="{{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">
                            <h4 class="font-bold text-gray-900 mb-1">{{ __('frontend.minimal_downtime') ?? 'توقف أدنى' }}</h4>
                            <p class="text-sm text-gray-600">{{ __('frontend.seamless_transfer') ?? 'نقل سلس' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <a href="#" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-600 to-teal-600 text-white font-semibold rounded-xl hover:from-green-700 hover:to-teal-700 transform hover:scale-105 transition-all duration-300 shadow-lg">
                    {{ __('frontend.start_migration') ?? 'ابدأ الانتقال' }}
                    <svg class="w-5 h-5 {{ app()->getLocale() == 'ar' ? 'mr-2' : 'ml-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ app()->getLocale() == 'ar' ? 'M15 19l-7-7 7-7' : 'M9 5l7 7-7 7' }}"></path>
                    </svg>
                </a>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            <div class="inline-flex items-center justify-center p-2 bg-blue-500/20 rounded-full mb-4">
                <span class="px-4 py-1 bg-blue-500 text-white text-sm font-semibold rounded-full">
                    {{ __('frontend.global_network') ?? 'شبكة عالمية' }}
                </span>
            </div>
            <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-6">
                {{ __('frontend.datacenter_locations') ?? 'أكثر من 20 موقع عالمي لمراكز البيانات' }}
            </h2>
            <p class="text-lg text-blue-100 max-w-4xl mx-auto leading-relaxed">
                {{ __('frontend.datacenter_desc') ?? 'نقوم بالنشر على مزودي السحابة العالميين لنوفر لك أكثر من 20 موقعاً عالمياً متصلاً بشكل مذهل. سيتم إعداد حساب الموزع الجديد الخاص بك على الفور ويكون جاهزاً للاستخدام في أقل من دقيقة!' }}
            </p>
        </div>
        ...
        <div class="relative overflow-hidden">
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="50">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.new_york') ?? 'New York' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.usa') ?? 'USA' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="100">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.los_angeles') ?? 'Los Angeles' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.usa') ?? 'USA' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="150">
                <div class="text-center mb-3"><span class="fi fi-us" style="font-size: 3rem; line-height: 1;"></span></div>
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.chicago') ?? 'Chicago' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.usa') ?? 'USA' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="200">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.london') ?? 'London' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.uk') ?? 'UK' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="250">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.amsterdam') ?? 'Amsterdam' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.netherlands') ?? 'Netherlands' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="300">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.frankfurt') ?? 'Frankfurt' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.germany') ?? 'Germany' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="350">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.paris') ?? 'Paris' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.france') ?? 'France' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="400">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.toronto') ?? 'Toronto' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.canada') ?? 'Canada' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="450">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.singapore_city') ?? 'Singapore' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.singapore') ?? 'Singapore' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="500">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.tokyo') ?? 'Tokyo' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.japan') ?? 'Japan' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="550">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.sydney') ?? 'Sydney' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.australia') ?? 'Australia' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="600">
                <div class="text-center mb-3"><span class="fi fi-au" style="font-size: 3rem; line-height: 1;"></span></div>
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.melbourne') ?? 'Melbourne' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.australia') ?? 'Australia' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="650">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.mumbai') ?? 'Mumbai' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.india') ?? 'India' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="700">
                <div class="text-center mb-3"><span class="fi fi-in" style="font-size: 3rem; line-height: 1;"></span></div>
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.bangalore') ?? 'Bangalore' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.india') ?? 'India' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="750">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.sao_paulo') ?? 'São Paulo' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.brazil') ?? 'Brazil' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="800">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.seoul') ?? 'Seoul' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.south_korea') ?? 'South Korea' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="850">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.madrid') ?? 'Madrid' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.spain') ?? 'Spain' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="900">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.milan') ?? 'Milan' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.italy') ?? 'Italy' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="950">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.stockholm') ?? 'Stockholm' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.sweden') ?? 'Sweden' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="1000">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.warsaw') ?? 'Warsaw' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.poland') ?? 'Poland' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="1050">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.dubai') ?? 'Dubai' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.uae') ?? 'UAE' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="1100">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.johannesburg') ?? 'Johannesburg' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.south_africa') ?? 'South Africa' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="1150">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.cairo') ?? 'Cairo' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.egypt') ?? 'Egypt' }}</p>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 border border-white/20" data-aos="fade-up" data-aos-delay="1200">
                ...
                <h4 class="text-white font-bold text-center mb-1">{{ __('frontend.riyadh') ?? 'Riyadh' }}</h4>
                <p class="text-blue-200 text-xs text-center">{{ __('frontend.saudi_arabia') ?? 'Saudi Arabia' }}</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-r from-blue-500/20 via-indigo-500/20 to-purple-500/20 backdrop-blur-sm rounded-2xl p-8 border border-white/20" data-aos="fade-up" data-aos-delay="1250">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                <div>
                    ...
                    <div class="text-blue-200 text-sm">{{ __('frontend.global_locations') ?? 'مواقع عالمية' }}</div>
                </div>
                <div>
                    <div class="text-4xl md:text-5xl font-bold text-white mb-2">&lt;1min</div>
                    <div class="text-blue-200 text-sm">{{ __('frontend.instant_setup') ?? 'إعداد فوري' }}</div>
                </div>
                <div>
                    <div class="text-4xl md:text-5xl font-bold text-white mb-2">99.9%</div>
                    <div class="text-blue-200 text-sm">{{ __('frontend.uptime_guarantee') ?? 'ضمان وقت التشغيل' }}</div>
                </div>
                <div>
                    <div class="text-4xl md:text-5xl font-bold text-white mb-2">24/7</div>
                    <div class="text-blue-200 text-sm">{{ __('frontend.network_monitoring') ?? 'مراقبة الشبكة' }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
                {{ __('frontend.faq_title') ?? 'الأسئلة الشائعة حول استضافة الموزعين' }}
            </h2>
            <p class="text-lg text-gray-600 max-w-3xl mx-auto">
                {{ __('frontend.faq_desc') ?? 'إجابات شاملة على جميع أسئلتك حول خدمة استضافة الموزعين' }}
            </p>
        </div>
        ...
        <div class="mb-8" data-aos="fade-up" data-aos-delay="100" x-data="{ activeTab: 'reselling' }">
            <div class="flex flex-wrap justify-center gap-4 mb-12">
                <button @click="activeTab = 'reselling'" 
                    ...
                    {{ __('frontend.tab_reselling') ?? 'الاستضافة' }}
                </button>
                <button @click="activeTab = 'migrations'" 
                        ...
                        class="px-6 py-3 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105">
                    {{ __('frontend.tab_migrations') ?? 'الانتقال' }}
                </button>
                <button @click="activeTab = 'resources'" 
                        ...
                        class="px-6 py-3 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105">
                    {{ __('frontend.tab_resources') ?? 'الموارد' }}
                </button>
            </div>
            ...
            <div x-show="activeTab === 'reselling'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_how_signup') ?? 'كيف أقوم بالتسجيل؟' }}</span>
                        ...
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_how_signup_answer') ?? 'يمكنك طلب استضافة الموزعين من خلال نظام الطلب الخاص بنا بشكل طبيعي. بعد الطلب، ستتمكن من تقديم إما مرجع المؤسسة الخيرية الخاص بك أو تفاصيل عن مؤسستك غير الربحية. نقوم بمراجعة هذا ثم إعداد الحساب؛ لا ينبغي أن يستغرق الأمر أكثر من بضع دقائق.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_support') ?? 'هل يمكنني استخدام دعم Pro Gineous؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_support_answer') ?? 'نعم، دعمنا الكامل 24/7 سعيد بالمساعدة. يمكننا أيضاً ترحيل موقعك من مزودي الخدمة الآخرين. نحن متاحون عبر الدردشة المباشرة والتذاكر والهاتف.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_technical_exp') ?? 'هل أحتاج إلى خبرة تقنية في تطوير المواقع أو إدارة الخوادم؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed mb-4">
                                {{ __('frontend.faq_technical_exp_answer') ?? 'كل ما نبيعه مصمم بحيث لا تحتاج إلى أي خبرة أو معرفة تقنية مسبقة. من حيث إدارة الخادم، نحن نعتني بكل شيء. سواء كنت تأخذ أصغر خطة استضافة Go لدينا، أو إذا ذهبت للحصول على خادم مخصص، فإن الأنظمة مُدارة بالكامل.' }}
                            </p>
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_technical_exp_answer2') ?? 'نقدم أكثر من 300 تطبيق مختلف لبناء المواقع يمكنك تثبيتها تلقائياً لبناء موقع بنفسك. الأكثر شعبية إلى حد بعيد هو WordPress، الذي يشغل أكثر من 35٪ من المواقع على الإنترنت ويعمل بشكل مثالي على Pro Gineous.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_client_features') ?? 'هل ستحتوي لوحة تحكم عميلي على نفس الميزات التي لدي؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_client_features_answer') ?? 'نعم، إلى حد ما. أنت فقط، الموزع لديك حق الوصول إلى لوحة الموزع الشاملة التي تسمح لك بالتحكم وإدارة جميع حسابات cPanel الخاصة بك. ثم لديك السيطرة على الميزات التي يمكن لكل حساب cPanel الوصول إليها، سواء بشكل فردي أو من خلال الباقات.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_manage_account') ?? 'كيف أقوم بإدارة حساب استضافة الموزع الخاص بي؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_manage_account_answer') ?? 'يأتي حساب الموزع الخاص بك مع لوحتي تحكم، cPanel و WHM. WHM هو لوحة تحكم الموزع الخاصة بك وهنا حيث تقوم بإدارة جميع حسابات cPanel الفرعية لعملائك. من هنا يمكنك إنشاء حسابات إضافية، وإدارة الموارد والقيود، وإضافة ميزات إضافية وهكذا.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_activation_time') ?? 'كم من الوقت يستغرق تفعيل باقة استضافة الموزع الخاصة بي؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_activation_time_answer') ?? 'بمجرد طلبك ودفعك لحساب استضافة الموزع الخاص بك، يجب أن تتلقى تفاصيل الترحيب الخاصة بك لتسجيل الدخول والبدء في غضون دقائق قليلة فقط. في حالات نادرة، قد نحتاج إلى الاتصال لمناقشة المزيد من التفاصيل معك إذا كان طلبك غير عادي قليلاً أو إذا كانت هناك أي معلومات مفقودة.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_what_is_reseller') ?? 'ما هي استضافة الموزعين وهل ستفيدني؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_payment_methods') ?? 'ما هي طرق الدفع التي تقبلونها؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_payment_methods_answer') ?? 'نقبل بطاقات الائتمان أو الخصم، PayPal، التحويل البنكي، والخصم المباشر. يجب أن يتم الدفع الأول ببطاقة ائتمان أو خصم، PayPal أو التحويل البنكي.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_money_back') ?? 'هل تقدمون ضمان استعادة الأموال؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_money_back_answer') ?? 'نعم، نفعل. تأتي خطط استضافة الويب واستضافة الموزعين لدينا مع ضمان استعادة الأموال لمدة 30 يوماً. يرجى ملاحظة أن الشروط تنطبق. يمكن لفرد واحد أو شركة أو مجموعة من الكيانات ذات الصلة استخدام ضمان استعادة الأموال مرة واحدة فقط.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_white_label') ?? 'هل استضافة Pro Gineous علامة بيضاء؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
            ...
            <div x-show="activeTab === 'migrations'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_can_migrate') ?? 'هل يمكن لـ Pro Gineous ترحيل مواقعي الحالية؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_can_migrate_answer') ?? 'نعم. لدينا فريق مخصص للهجرة سينقل مواقعك من مزود الخدمة الحالي الخاص بك إلى Pro Gineous. سنحتاج إلى الوصول إلى مزود الاستضافة القديم الخاص بك. بمجرد تقديم طلب للاستضافة، يرجى فتح تذكرة هجرة في منطقة العميل الخاصة بنا.' }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="activeTab === 'resources'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_customize_resources') ?? 'هل يمكنني تخصيص استخدام الموارد، مثل إضافة مساحة قرص إضافية؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_customize_resources_answer') ?? 'في خططنا غير المحدودة وغير المحدودة للموزعين، يمكنك اختيار عدد حسابات cPanel التي تريدها في حساب الموزع الخاص بك. إذا كنت على موزع Go الخاص بنا وتريد إضافة مساحة قرص إضافية، يجب عليك الترقية إلى موزع غير محدود 25. نحن لا نقدم باقات مخصصة بناءً على موزع Go.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_ssl_certificates') ?? 'هل يقدم Pro Gineous شهادات SSL؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_ssl_certificates_answer') ?? 'سيتم توفير كل نطاق ونطاق فرعي يشير إلى خادم Pro Gineous تلقائياً بشهادة SSL تغطي الموقع. بمجرد أن يشير نطاق إلينا، يعمل التشغيل الآلي الخاص بنا في غضون ساعة ويعين شهادة SSL للموقع. لا يوجد شيء تحتاج إلى القيام به، العملية آلية بالكامل.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_backup_services') ?? 'هل يقدم Pro Gineous خدمات النسخ الاحتياطي؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_backup_services_answer') ?? 'نعم، يقوم Pro Gineous بعمل نسخة احتياطية لجميع الملفات وقواعد البيانات تحت حسابك مرتين يومياً، ويخزنها لمدة 30 يوماً. لذلك في أي نقطة زمنية، هناك عدد كبير من نقاط الاستعادة للاختيار من بينها. هذه الخدمة مشمولة مجاناً في جميع باقات الاستضافة.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_emails_included') ?? 'كم عدد رسائل البريد الإلكتروني المضمنة؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_emails_included_answer') ?? 'يمكنك إنشاء العديد من صناديق البريد كما تريد. يمكن تعيين حد الإرسال إلى 1000 بريد إلكتروني في الساعة لكل حساب cPanel. إذا كنت بحاجة إلى إرسال أكثر من هذا، فيرجى إخبارنا ويمكننا المراجعة ولكن ضع في اعتبارك أننا لسنا مزود بريد إلكتروني جماعي.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_php_versions') ?? 'ما هي إصدارات PHP التي يمكنني استخدامها؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_php_versions_answer') ?? 'من خلال CloudLinux نوفر إصدارات PHP التاريخية المصححة، وأيضاً أحدث الإصدارات المستقرة. هذا يعني أنه يمكنك تشغيل كل شيء من PHP 5.2 إلى PHP 8. لتغيير إصدار PHP في حسابك، قم بتسجيل الدخول إلى cPanel وانقر على "إصدارات PHP" من هناك، يمكنك تحديد إصدار متاح من القائمة المنسدلة.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ open: false }">
                        <button @click="open = !open" class="w-full px-6 py-5 text-{{ app()->getLocale() == 'ar' ? 'right' : 'left' }} flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <span class="font-bold text-lg text-gray-900">{{ __('frontend.faq_upgrade_downgrade') ?? 'هل يمكنني ترقية أو تخفيض خطتي؟' }}</span>
                            <svg :class="open ? 'rotate-180' : ''" class="w-6 h-6 text-purple-600 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div x-show="open" x-transition class="px-6 pb-5">
                            <p class="text-gray-600 leading-relaxed">
                                {{ __('frontend.faq_upgrade_downgrade_answer') ?? 'نعم. من السهل الترقية و / أو التخفيض. يرجى فقط إعلام فريق الدعم لدينا، أو طلب الترقية من خلال النظام. بالنسبة للترقيات، سنقوم بتحصيل تكلفة الترقية بالتناسب حتى نهاية مدة الفوترة الحالية. بالنسبة للتخفيضات، يمكننا تخفيضك في نهاية مدتك.' }}
                            </p>
                        </div>
                    </div>
                </div>
...
<section class="py-16 mx-4 sm:mx-6 lg:mx-8 mb-16">
    <div class="max-w-5xl mx-auto bg-gradient-to-r from-purple-600 to-pink-600 rounded-3xl p-12 lg:p-16 text-center shadow-xl" data-aos="fade-up">
        <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-6">
            {{ app()->getLocale() == 'ar' ? 'هل أنت مستعد لبدء عملك؟' : 'Ready to Start Your Business?' }}
        </h2>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href="{{ route('home') }}" class="inline-flex items-center px-8 py-4 bg-transparent text-white font-bold text-lg rounded-xl border-2 border-white hover:bg-white hover:text-purple-600 transition-all duration-300">
