...
@section('title', __('frontend.checkout') ?? 'Checkout' . ' - ' . config('app.name'))
        ...
        <div class="max-w-5xl mx-auto">
            ...
            <div class="mb-8">
                <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">
                    {{ __('frontend.checkout') ?? 'Checkout' }}
                </h1>
                <p class="text-slate-500 dark:text-slate-400 mt-1">
                    {{ __('frontend.complete_your_order') ?? 'Complete your order securely' }}
                </p>
            </div>

            <form action="{{ route('cart.checkout.process') }}" method="POST" class="space-y-6">
                @csrf
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        ...
                        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
                            ...
                            <div class="flex items-center justify-between mb-5">
                                <h2 class="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-3">
                                    ...
                                    <span id="form-title">{{ __('frontend.contact_information') ?? 'Contact Information' }}</span>
                                </h2>
                                <button type="button" id="toggle-login-form" class="text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
                                    {{ __('frontend.already_have_account') ?? 'Already have an account?' }}
                                    <span class="font-medium text-slate-900 dark:text-white underline">{{ __('frontend.login') ?? 'Login' }}</span>
                                </button>
                            </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            ...
                                            <span class="text-sm text-slate-600 dark:text-slate-400">{{ __('frontend.remember_me') ?? 'Remember me' }}</span>
                                        </label>
                                        <a href="{{ route('login') }}" class="text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white">
                                            {{ __('frontend.forgot_password') ?? 'Forgot password?' }}
                                        </a>
                                    </div>
                                    <button type="button" id="login-submit-btn" class="w-full py-2.5 bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-slate-100 text-white dark:text-slate-900 font-medium rounded-lg transition-colors text-sm">
                                        <span id="login-btn-text">{{ __('frontend.login_and_continue') ?? 'Login & Continue' }}</span>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                    {{ __('frontend.username') }} <span class="text-red-500">*</span>
                                </label>
                                <div class="relative">
                                    <input type="text" id="username" name="username" required pattern="[A-Za-z0-9]+" minlength="3" maxlength="20" title="{{ __('frontend.username_format') }}" class="w-full px-3 py-2.5 pr-24 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-900 dark:focus:ring-white focus:border-transparent transition-shadow" placeholder="{{ __('frontend.username_placeholder') }}">
                                    
                                    <!-- Generate Username Button -->
                                    <button type="button" id="generate-username" class="absolute right-12 top-1/2 -translate-y-1/2 p-2 md:p-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors touch-manipulation" title="{{ __('frontend.generate_random_username') }}">
                            ...
                            <div class="grid md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">
                                        {{ __('frontend.first_name') }} <span class="text-red-500">*</span>
                                    </label>
                                    <input type="text" id="first_name" name="first_name" required pattern="[A-Za-z]+" maxlength="15" title="{{ __('frontend.english_letters_only') }}" class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                    <p id="first-name-message" class="mt-2 text-sm text-red-600 dark:text-red-400 hidden"></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">
                                        {{ __('frontend.last_name') }} <span class="text-red-500">*</span>
                                    </label>
                                    <input type="text" id="last_name" name="last_name" required pattern="[A-Za-z]+" maxlength="15" title="{{ __('frontend.english_letters_only') }}" class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                    {{ __('frontend.company_name') }} <span class="text-slate-400 text-xs">({{ __('frontend.optional') }})</span>
                                </label>
                                <input type="text" id="company_name" name="company_name" maxlength="30" pattern="[A-Za-z0-9\s]+" title="{{ __('frontend.english_letters_numbers_only') }}" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-500 focus:border-transparent text-sm">
                            ...
                            <div class="grid md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="email" id="email" name="email" required pattern="[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}" title="{{ __('frontend.english_letters_only') }}" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-500 focus:border-transparent text-sm">
                                    <p id="email-message" class="mt-1 text-xs text-red-600 dark:text-red-400 hidden"></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                        {{ __('frontend.phone_number') }} <span class="text-red-500">*</span>
                                    </label>
                                    <div class="flex" dir="ltr">
                                        <select id="country_code" name="country_code" required class="flex-shrink-0 w-20 px-2 py-2.5 text-xs border border-r-0 border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-l-lg focus:ring-2 focus:ring-slate-500 focus:border-transparent">
                                            ...
                                            @foreach($countriesData as $country)
                                                <option value="+{{ $country['code'] }}" {{ $country['iso'] == 'SA' ? 'selected' : '' }}>
                                                    +{{ $country['code'] }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="pt-4 border-t border-slate-200 dark:border-slate-700">
                                ...
                                <div class="mb-3">
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                        {{ __('frontend.address_2') }} <span class="text-slate-400 text-xs">({{ __('frontend.optional') }})</span>
                                    </label>
                                    <input type="text" id="address_2" name="address_2" maxlength="100" pattern="[A-Za-z0-9\s\.\-,#/]*" title="{{ __('frontend.english_letters_numbers_only') }}" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-500 focus:border-transparent text-sm" placeholder="{{ __('frontend.apartment_suite') }}">
                                ...
                                <div class="grid md:grid-cols-3 gap-3 mb-3">
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                            {{ __('frontend.city') }} <span class="text-red-500">*</span>
                                        </label>
                                        <input type="text" id="city" name="city" required maxlength="20" pattern="[A-Za-z\s\-]+" title="{{ __('frontend.english_letters_only') }}" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-500 focus:border-transparent text-sm">
                                        <p id="city-message" class="mt-1 text-xs text-red-600 dark:text-red-400 hidden"></p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                            {{ __('frontend.state_region') }} <span class="text-red-500">*</span>
                                        </label>
                                        <input type="text" id="state" name="state" required maxlength="20" pattern="[A-Za-z\s\-]+" title="{{ __('frontend.english_letters_only') }}" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-500 focus:border-transparent text-sm">
                                        <p id="state-message" class="mt-1 text-xs text-red-600 dark:text-red-400 hidden"></p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                            {{ __('frontend.postcode') }} <span class="text-red-500">*</span>
                                        </label>
                                        <input type="text" id="postcode" name="postcode" required maxlength="15" pattern="[A-Za-z0-9\s\-]+" title="{{ __('frontend.english_letters_numbers_only') }}" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-500 focus:border-transparent text-sm">
                                ...
                                <div class="mb-3">
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                        {{ __('frontend.country') }} <span class="text-red-500">*</span>
                                    </label>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                                        {{ __('frontend.tax_registration_number') }} <span class="text-slate-400 text-xs">({{ __('frontend.optional') }})</span>
                                    </label>
                                    <input type="text" id="tax_number" name="tax_number" maxlength="20" pattern="[A-Za-z0-9\-]*" title="{{ __('frontend.english_letters_numbers_only') }}" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-slate-500 focus:border-transparent text-sm" placeholder="{{ __('frontend.vat_number') }}">
                            ...
                            <div class="pt-4 border-t border-slate-200 dark:border-slate-700">
                                ...
                                <div class="grid md:grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p id="password-message" class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('frontend.password_requirements') }}</p>
                                    </div>
                        ...
                        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
                            <h2 class="text-lg font-semibold text-slate-900 dark:text-white mb-5 flex items-center gap-3">
                                ...
                                {{ __('frontend.payment_method') ?? 'Payment Method' }}
                            </h2>
                        
                        <!-- Payment Methods Grid -->
                        <div class="space-y-3">
                            {{-- Wallet Payment Option (Authenticated Users Only) --}}
                            @auth('client')
                                    ...
                                    <div class="payment-method-content relative flex items-center p-4 border-2 border-slate-200 dark:border-slate-700 rounded-lg transition-all hover:border-slate-400 dark:hover:border-slate-500">
                                        ...
                                        <div class="flex-1 ms-3">
                                            ...
                                            <div class="text-sm text-slate-500 dark:text-slate-400">
                                                {{ __('frontend.available') ?? 'Available' }}: ${{ number_format(auth('client')->user()->wallet_balance ?? 0, 2) }}
                                                @if((auth('client')->user()->wallet_balance ?? 0) < $total)
                                                    <span class="text-red-500 ms-1">• {{ __('frontend.insufficient_balance') ?? 'Insufficient' }}</span>
                                                @endif
                                            </div>
                                        </div>
                            ...
                            @if(isset($fawaterakPaymentMethods) && count($fawaterakPaymentMethods) > 0)
                                @foreach($fawaterakPaymentMethods as $method)
                                    <label class="payment-method-card block cursor-pointer">
                                        <input type="radio" name="payment_method" value="fawaterak_{{ $method['paymentId'] }}" class="payment-method-radio sr-only" required>
                                        <div class="payment-method-content relative flex items-center p-4 border-2 border-slate-200 dark:border-slate-700 rounded-lg transition-all hover:border-slate-400 dark:hover:border-slate-500">
                                            <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center">
                                                @if(!empty($method['logo']))
                                                    <img src="{{ $method['logo'] }}" alt="{{ $method['nameEn'] }}" class="max-w-full max-h-full object-contain">
                                                @else
                                            ...
                                            <div class="flex-1 ms-3">
                                                <div class="font-medium text-slate-900 dark:text-white">
                                                    {{ app()->getLocale() == 'ar' ? $method['nameAr'] : $method['nameEn'] }}
                                                </div>
                                            </div>
                                            <div class="payment-method-checkmark hidden text-slate-900 dark:text-white">
                                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            @else
                                <div class="text-center py-6 text-slate-500 dark:text-slate-400">
                                    <p>{{ __('frontend.no_payment_methods') ?? 'No payment methods available' }}</p>
                                </div>
                            @endif
                        </div>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 sticky top-24">
                        ...
                        <div class="space-y-3 mb-4 pb-4 border-b border-slate-200 dark:border-slate-700">
                                @foreach($cart as $item)
                                        ...
                                        <div class="flex justify-between items-start gap-2 mb-1">
                                            <div class="flex-1">
                                                ...
                                                @if(($item['type'] ?? 'domain') == 'hosting')
                                                    {{ $item['product_name'] ?? __('frontend.hosting') }}
                                                @elseif(($item['type'] ?? 'domain') == 'vps')
                                                    {{ $item['product_name'] ?? __('frontend.vps') }}
                                                @elseif(($item['type'] ?? 'domain') == 'dedicated')
                                                    {{ $item['product_name'] ?? __('frontend.dedicated_server') }}
                                                @else
                                                    {{ $item['domain'] ?? $item['product_name'] ?? 'Product' }}
                                                @endif
                                            </p>
                                    ...
                                    <div class="space-y-1 mt-2">
                                        @if(($item['type'] ?? 'domain') == 'hosting' || ($item['type'] ?? 'domain') == 'vps' || ($item['type'] ?? 'domain') == 'dedicated')
                                            {{-- Hosting/VPS/Dedicated Details --}}
                                            ...
                                            @if(isset($item['billing_cycle']))
                                                <div class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
                                                    ...
                                                    {{ __('frontend.' . $item['billing_cycle']) }}
                                                </div>
                                            @endif
                                            @if(isset($item['datacenter_name']) && $item['datacenter_name'])
                                                <div class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
                                                    ...
                                                    @if(isset($item['datacenter_price']) && $item['datacenter_price'] > 0)
                                                        <span class="text-purple-600 dark:text-purple-400">(+${{ number_format($item['datacenter_price'], 2) }})</span>
                                                    @endif
                                                </div>
                                            @endif
                                            {{-- Add-ons --}}
                                        ...
                                        @else
                                            ...
                                            @if(isset($item['years']) && $item['years'])
                                                <div class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
                                                    ...
                                                    {{ $item['years'] }} {{ $item['years'] > 1 ? __('frontend.years') : __('frontend.year') }}
                                                </div>
                                            @endif
                                            ...
                                            @if(isset($item['privacy']) && $item['privacy'])
                                                <div class="flex items-center gap-2 text-xs text-green-600 dark:text-green-400">
                                                    ...
                                                    {{ __('frontend.privacy_protection') }}
                                                </div>
                                            @endif
                                        @endif
                                        ...
                                        @if(isset($item['quantity']) && $item['quantity'] > 1)
                                            <div class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
                                                ...
                                                {{ __('frontend.quantity') }}: {{ $item['quantity'] }}
                                            </div>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="flex justify-between items-center mb-4">
                            <span class="font-semibold text-slate-900 dark:text-white">{{ __('frontend.total') ?? 'Total' }}</span>
                            <span class="text-xl font-bold text-slate-900 dark:text-white">${{ number_format($total, 2) }}</span>
                        </div>

                        <!-- Terms and Conditions Agreement - Show only if user hasn't accepted before -->
                        @if(!Auth::guard('client')->check() || !Auth::guard('client')->user()->hasAcceptedTerms())
                        <div class="mb-4 pb-4 border-b border-slate-200 dark:border-slate-700">
                            <label class="flex items-start gap-2 cursor-pointer">
                                ...
                                <span class="flex-1 text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                                    {{ __('frontend.i_agree_to_the') ?? 'I agree to the' }}
                                    <a href="{{ route('terms') }}" target="_blank" class="text-slate-900 dark:text-white hover:underline font-medium">
                                        {{ __('frontend.terms_and_conditions') ?? 'Terms and Conditions' }}
                                    </a>
                                    {{ __('frontend.and') ?? 'and' }}
                                    <a href="{{ route('privacy') }}" target="_blank" class="text-slate-900 dark:text-white hover:underline font-medium">
                                        {{ __('frontend.privacy_policy') ?? 'Privacy Policy' }}
                                    </a>
                                </span>
                            </label>
                        ...
                        <button type="submit" id="place-order-btn" class="w-full py-3 text-sm bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-slate-100 text-white dark:text-slate-900 font-medium rounded-lg transition-colors">
                            <span class="flex items-center justify-center gap-2">
                                ...
                                {{ __('frontend.place_order') ?? 'Place Order' }}
                            </span>
                        </button>
                        
                        <a href="{{ route('cart.index') }}" class="block w-full py-2.5 text-sm bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 text-center font-medium rounded-lg transition-colors mt-2">
                            {{ __('frontend.back_to_cart') ?? 'Back to Cart' }}
                        </a>
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 space-y-2">
                            <div class="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                                ...
                                <span>{{ __('frontend.secure_payment') ?? 'Secure Payment' }}</span>
                            </div>
                            <div class="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                                </svg>
                                <span>{{ __('frontend.ssl_encrypted') ?? 'SSL Encrypted' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (toggleLoginBtn) {
        toggleLoginBtn.addEventListener('click', function() {
            ...
            if (isLoginMode) {
                ...
                formTitle.textContent = '{{ __("frontend.login") ?? "Login" }}';
                toggleLoginBtn.innerHTML = '{{ __("frontend.new_customer") ?? "New customer?" }} <span class="font-medium text-slate-900 dark:text-white underline">{{ __("frontend.create_account") ?? "Create account" }}</span>';
                ...
                formTitle.textContent = '{{ __("frontend.contact_information") ?? "Contact Information" }}';
                toggleLoginBtn.innerHTML = '{{ __("frontend.already_have_account") ?? "Already have an account?" }} <span class="font-medium text-slate-900 dark:text-white underline">{{ __("frontend.login") ?? "Login" }}</span>';
    ...
    if (loginSubmitBtn) {
        loginSubmitBtn.addEventListener('click', async function() {
            ...
            if (!email || !password) {
                loginError.textContent = '{{ __("frontend.please_fill_all_fields") ?? "Please fill all fields" }}';
            ...
            try {
                ...
                if (response.ok && data.success) {
                    ...
                    loginError.textContent = data.message || '{{ __("frontend.invalid_credentials") ?? "Invalid email or password" }}';
                    loginError.classList.remove('hidden');
                }
            } catch (error) {
                loginError.textContent = '{{ __("frontend.login_error") ?? "An error occurred. Please try again." }}';
                loginError.classList.remove('hidden');
            } finally {
    ...
    async function generateUniqueUsername() {
        ...
        while (attempts < maxAttempts) {
            ...
            try {
                ...
                if (data.available) {
                    ...
                    showSuccess('{{ __("frontend.username_available") }}');
        ...
        if (usernameSpinner) usernameSpinner.classList.add('hidden');
        showError('{{ __("frontend.username_check_error") }}');
    }
    ...
    if (usernameInput) {
        ...
        if (username.length < 3) {
            showError('{{ __("frontend.username_too_short") }}');
            return;
        }
        
        if (username.length > 20) {
            showError('{{ __("frontend.username_too_long") }}');
            return;
        }
        
        if (!/^[A-Za-z0-9]+$/.test(username)) {
            showError('{{ __("frontend.username_invalid_format") }}');
            return;
        }
    ...
    function checkUsernameAvailability(username) {
        ...
        .then(data => {
            ...
            if (data.available) {
                ...
                showError('{{ __("frontend.username_taken") }}');
                isUsernameAvailable = false;
            }
        })
        .catch(error => {
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                if (usernameValue && !isUsernameAvailable) {
                    ...
                    showError('{{ __("frontend.username_must_be_available") }}');
                    usernameInput.focus();
                }
            });
        }
    ...
    if (firstNameInput && firstNameMessage) {
        firstNameInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                firstNameMessage.textContent = '{{ __("frontend.english_letters_only") }}';
    ...
    if (lastNameInput && lastNameMessage) {
        lastNameInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                lastNameMessage.textContent = '{{ __("frontend.english_letters_only") }}';
    ...
    if (companyNameInput && companyNameMessage) {
        companyNameInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                companyNameMessage.textContent = '{{ __("frontend.english_letters_numbers_only") }}';
    ...
    if (emailInput && emailMessage) {
        emailInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                emailMessage.textContent = '{{ __("frontend.english_letters_only") }}';
        ...
        emailCheckTimeout = setTimeout(async () => {
            try {
                ...
                if (data.available) {
                    ...
                    emailMessage.textContent = '{{ __("frontend.email_available") }}';
                    ...
                    emailInput.classList.add('border-red-500', 'dark:border-red-500');
                    emailMessage.textContent = '{{ __("frontend.email_already_used") }}';
    ...
    if (phoneInput && phoneMessage) {
        phoneInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                phoneMessage.textContent = '{{ __("frontend.numbers_only") }}';
    ...
    async function checkPhoneAvailability() {
        ...
        phoneCheckTimeout = setTimeout(async () => {
            try {
                const response = await fetch('{{ route("api.check-phone") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                ...
                if (data.available) {
                    ...
                    phoneMessage.textContent = '{{ __("frontend.phone_available") }}';
                    ...
                    phoneInput.classList.add('border-red-500', 'dark:border-red-500');
                    phoneMessage.textContent = '{{ __("frontend.phone_already_used") }}';
    ...
    if (address1Input && address1Message) {
        address1Input.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                address1Message.textContent = '{{ __("frontend.english_letters_numbers_only") }}';
    ...
    if (address2Input && address2Message) {
        address2Input.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                address2Message.textContent = '{{ __("frontend.english_letters_numbers_only") }}';
    ...
    if (cityInput && cityMessage) {
        cityInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                cityMessage.textContent = '{{ __("frontend.english_letters_only") }}';
    ...
    if (stateInput && stateMessage) {
        stateInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                stateMessage.textContent = '{{ __("frontend.english_letters_only") }}';
    ...
    if (postcodeInput && postcodeMessage) {
        postcodeInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                postcodeMessage.textContent = '{{ __("frontend.english_letters_numbers_only") }}';
    ...
    if (taxNumberInput && taxNumberMessage) {
        taxNumberInput.addEventListener('input', function(e) {
            ...
            if (value !== cleaned) {
                ...
                // Show warning message
                taxNumberMessage.textContent = '{{ __("frontend.english_letters_numbers_only") }}';
    ...
    async function checkTaxNumberAvailability() {
        ...
        taxNumberCheckTimeout = setTimeout(async () => {
            try {
                const response = await fetch('{{ route("api.check-tax-number") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                ...
                if (data.available) {
                    ...
                    taxNumberMessage.textContent = '{{ __("frontend.tax_number_available") }}';
                    ...
                    taxNumberInput.classList.add('border-red-500', 'dark:border-red-500');
                    taxNumberMessage.textContent = '{{ __("frontend.tax_number_already_used") }}';
    ...
    passwordInput?.addEventListener('input', function() {
        ...
        if (originalValue !== englishValue) {
            ...
            passwordMessage.textContent = '{{ __("frontend.english_characters_only") }}';
            ...
            passwordTimeout = setTimeout(() => {
                if (this.value.length >= 8) {
                    ...
                    passwordMessage.textContent = '{{ __("frontend.password_requirements") }}';
        ...
        if (value.length >= 8) {
            ...
            passwordMessage.textContent = '{{ __("frontend.password_strong") }}';
    ...
    passwordConfirmInput?.addEventListener('input', function() {
        ...
        if (originalValue !== englishValue) {
            this.value = englishValue;
            this.classList.add('border-red-500', 'dark:border-red-500');
            passwordConfirmMessage.textContent = '{{ __("frontend.english_characters_only") }}';
    ...
    function checkPasswordMatch() {
        ...
        if (password === confirmPassword) {
            ...
            passwordConfirmMessage.textContent = '{{ __("frontend.passwords_match") }}';
            ...
            passwordConfirmMessage.textContent = '{{ __("frontend.passwords_dont_match") }}';
    ...
    if (paymentMethodRadios.length > 0) {
        ...
        @auth('client')
        const walletBalance = {{ auth('client')->user()->wallet_balance ?? 0 }};
    ...
    if (placeOrderBtn) {
        ...
        placeOrderBtn.addEventListener('click', function(e) {
            ...
            if (termsCheckbox && !termsCheckbox.checked) {
                ...
                termsError.textContent = '{{ __("frontend.please_accept_terms") ?? "Please accept the terms and conditions" }}';
            ...
            const loadingText = '{{ __("frontend.processing") ?? "Processing..." }}';
            this.disabled = true;
    ...
    window.onTurnstileError = function(error) {
        ...
        alert('{{ app()->getLocale() == "ar" ? "فشل التحقق الأمني. يرجى إعادة المحاولة." : "Security verification failed. Please try again." }}');
    };
});
</script>
