...
@section('title', __('frontend.shared_hosting') . ' - ' . config('app.name'))
    ...
    <div class="container mx-auto px-4 max-w-7xl">
        ...
        <div class="mb-6">
            ...
            <p class="text-slate-600 dark:text-slate-400">
                {{ __('frontend.manage_your_hosting_services') ?? 'Manage all your hosting services from one place' }}
            </p>
        </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="p-6 border-b border-slate-200 dark:border-slate-700">
                    <h2 class="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        ...
                        {{ __('frontend.your_hosting_services') ?? 'Your Hosting Services' }}
                    </h2>
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
                        <span class="font-semibold text-blue-600">{{ $hostingServices->total() }}</span> 
                        {{ __('frontend.services_found') ?? 'services found' }}
                    </p>
                </div>
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">
                                    {{ __('frontend.created_date') ?? 'Created' }}
                                </th>
                            ...
                            <tr class="service-row border-b border-slate-100 dark:border-slate-700 last:border-0">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p class="text-sm font-bold text-gray-900">{{ $service->service_name }}</p>
                                            <p class="text-xs text-gray-500 mt-1">#{{ $service->order_id }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5 text-right">
                                    @if($service->status === 'terminated')
                                        <span class="inline-flex items-center gap-2 px-5 py-2.5 bg-gray-300 text-gray-500 text-sm font-semibold rounded-lg cursor-not-allowed">
                                            ...
                                            {{ __('frontend.terminated') ?? 'Terminated' }}
                                        </span>
                                    @else
                                        <a href="{{ route('client.hosting.show', $service->id) }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-200">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                            </svg>
                                            {{ __('frontend.view_details') ?? 'View Details' }}
                                        </a>
                                    @endif
                                </td>
                            </tr>
                    ...
                    <div class="p-5 service-item">
                        <div class="flex items-start gap-3 mb-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <h3 class="text-base font-bold text-gray-900 mb-0.5">{{ $service->service_name }}</h3>
                                <p class="text-xs text-gray-500">#{{ $service->order_id }}</p>
                            </div>
                            @if($service->status === 'active')
                        ...
                        <div class="bg-gray-50 rounded-xl p-4 mb-4 space-y-2.5">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-semibold text-gray-900">{{ $service->domain ?? 'N/A' }}</span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-gray-600 flex items-center gap-2">
                                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            <a href="{{ route('client.hosting.show', $service->id) }}" class="flex items-center justify-center gap-2 w-full px-5 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-200">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                </svg>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-12 text-center">
                <div class="max-w-md mx-auto">
                    ...
                    <h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-3">
                        {{ __('frontend.no_hosting_services') ?? 'No Hosting Services' }}
                    </h3>
                    <p class="text-slate-600 dark:text-slate-400 mb-6">
                        {{ __('frontend.no_hosting_services_description') ?? 'You don\'t have any hosting services yet. Start by ordering a new service.' }}
                    </p>
                    
                    <a href="{{ route('home') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                        </svg>
                        {{ __('frontend.order_hosting') ?? 'Order Hosting' }}
                    </a>
                </div>
            </div>
        @endif
