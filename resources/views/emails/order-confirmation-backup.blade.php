...
{{-- Logo & Header --}}
<div style="text-align: center; padding-bottom: 25px; border-bottom: 2px solid #e2e8f0; margin-bottom: 25px; direction: {{ $direction }};">
...
<h1 style="color: #1a202c; font-size: 26px; font-weight: 700; margin: 0; direction: {{ $direction }};">{{ __('emails.order_confirmed') ?? 'Order Confirmed' }}</h1>
<p style="color: #718096; font-size: 15px; margin-top: 10px; margin-bottom: 0; direction: {{ $direction }};">{{ __('emails.thank_you_message') ?? 'Thank you for your purchase' }}</p>
</div>
...
{{-- Order Info Badge --}}
<div style="background: linear-gradient(135deg, #1d71b8 0%, #1a5a94 100%); border-radius: 12px; padding: 20px; margin: 25px 0; color: #ffffff;">
<table style="width: 100%; border-collapse: collapse; direction: {{ $direction }};" cellpadding="0" cellspacing="0">
<tr>
<td style="vertical-align: middle; padding: 5px 10px; text-align: {{ $textAlign }};">
...
<td style="text-align: {{ $textAlignOpposite }}; vertical-align: middle; padding: 5px 10px;">
...
<td style="padding: 10px 0; text-align: {{ $textAlignOpposite }}; color: #1a202c; border-bottom: 1px solid #f0f0f0;">{{ ucfirst($order->payment_method ?? 'N/A') }}</td>
</tr>
<tr>
...
<span style="background-color: #c6f6d5; color: #22543d; padding: 5px 14px; border-radius: 20px; font-size: 12px; font-weight: 600; text-transform: uppercase;">{{ $order->payment_status }}</span>
</td>
</tr>
</table>
</div>
...
<div style="display: flex; justify-content: space-between; padding: 15px; background-color: #f8fafc; border-radius: 8px; margin-bottom: 10px;">
<table style="width: 100%; direction: {{ $direction }};">
<tr>
<td style="vertical-align: top; text-align: {{ $textAlign }};">
<p style="margin: 0; font-weight: 600; color: #1a202c; font-size: 14px;">{{ $item->product_name }}</p>
@if(isset($item->configuration['domain']))
<p style="margin: 5px 0 0 0; color: #718096; font-size: 13px;">{{ $item->configuration['domain'] }}</p>
@endif
</td>
<td style="text-align: {{ $textAlignOpposite }}; vertical-align: top;">
...
<p style="margin: 0 0 10px 0; font-size: 15px; color: #92400e; font-weight: 600;">{{ __('emails.rate_experience') ?? 'How was your experience?' }}</p>
<p style="margin: 0 0 15px 0; font-size: 13px; color: #a16207;">{{ __('emails.rate_experience_desc') ?? 'Your feedback helps us improve our services.' }}</p>
...
<p style="color: #718096; font-size: 13px; margin: 0 0 15px 0;">{{ __('emails.questions') ?? 'If you have any questions, please don\'t hesitate to contact our support team.' }}</p>

{{-- Signature --}}
<div style="margin: 20px 0;">
<p style="color: #4a5568; font-size: 14px; margin: 0;">{{ __('emails.best_regards') ?? 'Best regards' }},</p>
<p style="color: #1d71b8; font-size: 15px; font-weight: 600; margin: 8px 0 3px 0;">{{ __('emails.customer_success_team') ?? 'Customer Success Team' }}</p>
<p style="color: #1a202c; font-size: 14px; font-weight: 700; margin: 0;">{{ config('app.name') }}</p>
</div>

{{-- Company Info --}}
<div style="background-color: #f8fafc; border-radius: 8px; padding: 20px; margin-top: 20px; text-align: {{ $textAlign }};">
<p style="margin: 0 0 15px 0; font-size: 13px; color: #1a202c; font-weight: 700; text-align: center;">{{ config('app.name') }}</p>

{{-- UK Office --}}
<div style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #e2e8f0;">
<p style="margin: 0 0 5px 0; font-size: 11px; color: #1d71b8; font-weight: 600;">{{ __('emails.uk_office') ?? 'UK Office' }}</p>
<p style="margin: 0 0 3px 0; font-size: 11px; color: #64748b; line-height: 1.5;">71-75, Shelton Street, Covent Garden, London, WC2H 9JQ, United Kingdom</p>
<p style="margin: 0; font-size: 10px; color: #94a3b8;">{{ __('emails.reg_number') ?? 'Reg. No' }}: 16307182</p>
</div>

{{-- Egypt Office --}}
<div style="margin-bottom: 15px;">
<p style="margin: 0 0 5px 0; font-size: 11px; color: #1d71b8; font-weight: 600;">{{ __('emails.egypt_office') ?? 'Egypt Office' }}</p>
<p style="margin: 0 0 3px 0; font-size: 11px; color: #64748b; line-height: 1.5;">Bani Waldin Ihsanan Tower - 3rd Floor, Mostafa Kamel Street, Beni Suef Center, Beni Suef Governorate</p>
<p style="margin: 0 0 2px 0; font-size: 10px; color: #94a3b8;">{{ __('emails.reg_number') ?? 'Reg. No' }}: 90088</p>
<p style="margin: 0; font-size: 10px; color: #94a3b8;">{{ __('emails.tax_number') ?? 'Tax Reg. No' }}: 755-552-334</p>
</div>
...
<x-mail::subcopy>
{{ __('emails.order_reference') ?? 'Order Reference' }}: #{{ $order->order_number }} | {{ __('emails.invoice_reference') ?? 'Invoice' }}: #{{ $invoice->invoice_number }}
</x-mail::subcopy>
</x-mail::message>
