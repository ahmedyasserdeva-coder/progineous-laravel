...
<?php $__env->startSection('title', $service->service_name . ' - ' . ($isArabic ?? false ? 'إدارة استضافة الموزعين' : 'Reseller Hosting Management')); ?>
...
<div class="container mx-auto px-4 py-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3 rtl:space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="text-sm font-medium text-gray-500"><?php echo e(__('crm.reseller_hosting') ?? ($isArabic ? 'استضافة الموزعين' : 'Reseller Hosting')); ?></span>
                </div>
            </li>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 mb-6 shadow-sm">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="flex items-center gap-5">
                <div class="w-14 h-14 bg-gradient-to-br from-slate-600 to-slate-800 rounded-2xl flex items-center justify-center shadow-lg shadow-slate-600/20">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="https://<?php echo e($service->server->hostname); ?>:2087/login/?user=<?php echo e($service->username); ?>&pass=<?php echo e(urlencode($service->decrypted_password)); ?>" target="_blank" 
                   class="inline-flex items-center gap-2 px-4 py-2.5 text-white bg-gradient-to-r from-slate-600 to-slate-700 rounded-xl hover:from-slate-700 hover:to-slate-800 transition-all text-sm font-medium shadow-lg shadow-slate-600/25">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"/>
                    </svg>
                    <?php echo e($isArabic ? 'دخول WHM' : 'WHM Login'); ?>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        ...
        <button type="button" onclick="document.getElementById('changePasswordModal').classList.remove('hidden')" 
                class="group flex flex-col items-center gap-2 p-4 bg-white border border-gray-200 rounded-xl hover:border-slate-300 hover:bg-slate-50/50 transition-all">
        ...
        <!-- Activate -->
        <form action="<?php echo e(route('admin.services.update-status', $service)); ?>" method="POST" class="contents">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="status" value="active">
            <button type="submit" class="group flex flex-col items-center gap-2 p-4 bg-white border border-gray-200 rounded-xl hover:border-emerald-300 hover:bg-emerald-50/50 transition-all">
                ...
                <span class="text-xs font-medium text-gray-700 text-center"><?php echo e($isArabic ? 'تفعيل' : 'Activate'); ?></span>
            </button>
        </form>
        <?php endif; ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
                    ...
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-xl">
                            <div>
                                ...
                                <dd class="mt-1 text-sm font-semibold text-gray-900"><?php echo e(__('crm.reseller_hosting') ?? ($isArabic ? 'استضافة الموزعين' : 'Reseller Hosting')); ?></dd>
                            </div>
                        </div>
                        <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-xl hover:bg-gray-50 transition-colors">
                            <div>
                        ...
                        <?php
                            $cpanelAccounts = null;
                            // Try to get cpanel_accounts from orderItem configuration
                            if ($service->orderItem && isset($service->orderItem->configuration['cpanel_accounts'])) {
                                $cpanelAccounts = $service->orderItem->configuration['cpanel_accounts'];
                            } elseif ($service->server_data && isset($service->server_data['cpanel_accounts'])) {
                                $cpanelAccounts = $service->server_data['cpanel_accounts'];
                            }
                        ?>
                        <?php if($cpanelAccounts): ?>
                        <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-xl hover:bg-gray-50 transition-colors">
                            <div>
                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide"><?php echo e($isArabic ? 'حسابات cPanel' : 'cPanel Accounts'); ?></dt>
                                <dd class="mt-1 text-sm font-semibold text-gray-900 flex items-center gap-2">
                                    <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                    <?php echo e($cpanelAccounts); ?> <?php echo e($isArabic ? 'حساب' : 'Accounts'); ?>

                                </dd>
                            </div>
                            <button type="button" onclick="document.getElementById('changeCpanelAccountsModal').classList.remove('hidden')" 
                                    class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                </svg>
            ...
            <!-- WHM Credentials Section - For Manual Setup -->
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-slate-100/50">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-base font-semibold text-slate-900"><?php echo e($isArabic ? 'بيانات الدخول لـ WHM' : 'WHM Login Credentials'); ?></h3>
                        </div>
                        <button type="button" onclick="document.getElementById('editWhmCredentialsModal').classList.remove('hidden')" 
                                class="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-slate-800 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                            <?php echo e($isArabic ? 'تعديل' : 'Edit'); ?>
                ...
                <div class="p-6">
                    <?php
                        ...
                        $whmLoginUrl = $metadata['whm_login_url'] ?? $metadata['login_url'] ?? null;
                        $whmUsername = $service->username ?? $metadata['whm_username'] ?? null;
                        $whmPassword = $service->decrypted_password ?? $metadata['whm_password'] ?? null;
                        $hasWhmCredentials = !empty($whmLoginUrl) && !empty($whmUsername) && !empty($whmPassword);
                    ?>
                    
                    <?php if(!$hasWhmCredentials): ?>
                    <div class="text-center py-6">
                        ...
                        <h4 class="text-lg font-semibold text-gray-900 mb-2"><?php echo e($isArabic ? 'بيانات الدخول غير مكتملة' : 'Credentials Not Set'); ?></h4>
                        <p class="text-gray-500 mb-4"><?php echo e($isArabic ? 'يرجى إضافة بيانات الدخول لـ WHM ليتمكن العميل من الوصول لحسابه' : 'Please add WHM login credentials so the client can access their account'); ?></p>
                        <button type="button" onclick="document.getElementById('editWhmCredentialsModal').classList.remove('hidden')" 
                                class="inline-flex items-center gap-2 px-4 py-2 text-white bg-slate-700 rounded-lg hover:bg-slate-800 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
                            <?php echo e($isArabic ? 'إضافة بيانات الدخول' : 'Add Credentials'); ?>
                    ...
                    <dl class="space-y-4">
                        <div class="p-4 bg-gray-50 rounded-xl">
                            <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2"><?php echo e($isArabic ? 'رابط الدخول' : 'Login URL'); ?></dt>
                            <dd class="flex items-center gap-2">
                                <code class="flex-1 text-sm font-mono text-slate-700 bg-white px-3 py-2 rounded-lg border border-gray-200 overflow-x-auto"><?php echo e($whmLoginUrl); ?></code>
                                <button onclick="navigator.clipboard.writeText('<?php echo e($whmLoginUrl); ?>')" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Copy">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                </button>
                                <a href="<?php echo e($whmLoginUrl); ?>" target="_blank" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Open">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="p-4 bg-gray-50 rounded-xl">
                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2"><?php echo e($isArabic ? 'اسم المستخدم' : 'Username'); ?></dt>
                                <dd class="flex items-center gap-2">
                                    <code class="flex-1 text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg border border-gray-200"><?php echo e($whmUsername); ?></code>
                                    <button onclick="navigator.clipboard.writeText('<?php echo e($whmUsername); ?>')" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Copy">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                    </button>
                                </dd>
                            </div>
                            <div class="p-4 bg-gray-50 rounded-xl" x-data="{ showPassword: false }">
                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2"><?php echo e($isArabic ? 'كلمة المرور' : 'Password'); ?></dt>
                                <dd class="flex items-center gap-2">
                                    <code class="flex-1 text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg border border-gray-200">
                                        <span x-show="!showPassword">••••••••••••</span>
                                        <span x-show="showPassword"><?php echo e($whmPassword); ?></span>
                                    </code>
                                    <button @click="showPassword = !showPassword" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Toggle">
                                        <svg x-show="!showPassword" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        <svg x-show="showPassword" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                                    </button>
                                    <button onclick="navigator.clipboard.writeText('<?php echo e($whmPassword); ?>')" class="p-2 text-gray-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-colors" title="Copy">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                    </button>
                                </dd>
                            </div>
                            ...
                            <?php $__currentLoopData = $invoices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                    $statusConfig = [
                                        ...
                                        'refunded' => ['bg' => 'bg-slate-50', 'text' => 'text-slate-800', 'ring' => 'ring-slate-700/20', 'icon' => 'text-slate-600'],
                                    ];
    ...
    <!-- Suspend Modal -->
                ...
                <div class="fixed inset-0 flex items-center justify-center">
                    <div class="relative bg-white rounded-lg shadow-xl w-96 mx-4 p-5 z-10" onclick="event.stopPropagation()">
                        ...
                        <form action="<?php echo e(route('admin.services.change-password', $service)); ?>" method="POST" x-data="{ password: '', showPassword: false }">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="relative">
                                    <input :type="showPassword ? 'text' : 'password'" name="password" x-model="password" required minlength="8"
                                           class="w-full px-4 py-2.5 pe-12 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600"
                            ...
                            <div class="mb-4">
                                <button type="button" @click="password = '<?php echo e(\Illuminate\Support\Str::random(16)); ?>'" class="text-sm text-slate-700 hover:text-slate-900">
                            ...
                            <div class="flex gap-3">
                                ...
                                <button type="submit" 
                                        class="flex-1 px-4 py-2 text-sm font-medium text-white bg-slate-600 rounded-lg hover:bg-slate-700 transition-colors">
            ...
            <!-- Change cPanel Accounts Modal -->
            <div id="changeCpanelAccountsModal" class="fixed inset-0 z-50 hidden" onclick="this.classList.add('hidden')">
                <!-- Backdrop -->
                <div class="fixed inset-0 bg-gray-500/75 transition-opacity"></div>
                
                <!-- Modal Content -->
                <div class="fixed inset-0 flex items-center justify-center">
                    <div class="relative bg-white rounded-lg shadow-xl w-[500px] mx-4 p-5 z-10" onclick="event.stopPropagation()">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                <svg class="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                </svg>
                                <?php echo e($isArabic ? 'تغيير عدد حسابات cPanel' : 'Change cPanel Accounts'); ?>

                            </h3>
                            <button type="button" onclick="document.getElementById('changeCpanelAccountsModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                            </button>
                        </div>
                        <form action="<?php echo e(route('admin.services.update-cpanel-accounts', $service)); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('PUT'); ?>
                            <?php
                                $currentCpanelAccounts = $service->orderItem->configuration['cpanel_accounts'] ?? $service->server_data['cpanel_accounts'] ?? 0;
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    <?php echo e($isArabic ? 'العدد الحالي' : 'Current Count'); ?>

                                </label>
                                <input type="text" value="<?php echo e($currentCpanelAccounts); ?> <?php echo e($isArabic ? 'حساب' : 'Accounts'); ?>" readonly
                                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-100 text-gray-600">
                            </div>
                            
                            <?php if(isset($cpanelTiers) && $cpanelTiers->count() > 0): ?>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    <?php echo e($isArabic ? 'اختر باقة cPanel' : 'Select cPanel Tier'); ?> <span class="text-red-500">*</span>
                                </label>
                                <div class="space-y-2">
                                    <?php if(isset($product) && $product->base_cpanel_accounts): ?>
                                    <label class="flex items-center justify-between p-3 border rounded-lg cursor-pointer hover:bg-blue-50 transition-colors <?php echo e($currentCpanelAccounts == $product->base_cpanel_accounts ? 'border-blue-500 bg-blue-50' : 'border-gray-200'); ?>">
                                        <div class="flex items-center gap-3">
                                            <input type="radio" name="cpanel_accounts" value="<?php echo e($product->base_cpanel_accounts); ?>" 
                                                   class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                                                   <?php echo e($currentCpanelAccounts == $product->base_cpanel_accounts ? 'checked' : ''); ?>>
                                            <div>
                                                <span class="font-medium text-gray-900"><?php echo e($product->base_cpanel_accounts); ?> <?php echo e($isArabic ? 'حساب' : 'Accounts'); ?></span>
                                                <span class="text-xs text-gray-500 <?php echo e($isArabic ? 'mr-2' : 'ml-2'); ?>">(<?php echo e($isArabic ? 'الأساسي' : 'Base'); ?>)</span>
                                            </div>
                                        </div>
                                        <span class="text-sm font-semibold text-emerald-600"><?php echo e($isArabic ? 'مجاني' : 'Free'); ?></span>
                                    </label>
                                    <?php endif; ?>
                                    
                                    <?php $__currentLoopData = $cpanelTiers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tier): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php
                                        $tierPrice = $tier->$priceColumn ?? $tier->monthly_price;
                                    ?>
                                    <label class="flex items-center justify-between p-3 border rounded-lg cursor-pointer hover:bg-blue-50 transition-colors <?php echo e($currentCpanelAccounts == $tier->tier ? 'border-blue-500 bg-blue-50' : 'border-gray-200'); ?>">
                                        <div class="flex items-center gap-3">
                                            <input type="radio" name="cpanel_accounts" value="<?php echo e($tier->tier); ?>" 
                                                   class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                                                   <?php echo e($currentCpanelAccounts == $tier->tier ? 'checked' : ''); ?>>
                                            <div>
                                                <span class="font-medium text-gray-900"><?php echo e($tier->tier); ?> <?php echo e($isArabic ? 'حساب' : 'Accounts'); ?></span>
                                            </div>
                                        </div>
                                        <span class="text-sm font-semibold text-blue-600">+<?php echo e(number_format($tierPrice, 2)); ?> <?php echo e(__('crm.currency')); ?>/<?php echo e(__('crm.' . str_replace('-', '_', $billingCycle))); ?></span>
                                    </label>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    <?php echo e($isArabic ? 'العدد الجديد' : 'New Count'); ?> <span class="text-red-500">*</span>
                                </label>
                                <input type="number" name="cpanel_accounts" required min="1" max="1000"
                                       value="<?php echo e($currentCpanelAccounts); ?>"
                                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <p class="mt-1 text-xs text-gray-500"><?php echo e($isArabic ? 'أدخل عدد حسابات cPanel المسموح بها للعميل' : 'Enter the number of cPanel accounts allowed for the client'); ?></p>
                            </div>
                            <?php endif; ?>
                            ...
                            <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                                <p class="text-xs text-amber-700">
                                    ...
                                    <?php echo e($isArabic ? 'ملاحظة: هذا سيغير العدد في قاعدة البيانات فقط. يجب عليك تحديث حدود الحساب في WHM يدوياً إذا لزم الأمر.' : 'Note: This will only change the count in the database. You may need to update the account limits in WHM manually if required.'); ?>

                                </p>
                            </div>
                            <div class="flex gap-3">
                                <button type="button" onclick="document.getElementById('changeCpanelAccountsModal').classList.add('hidden')" 
                                        class="flex-1 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
                                    <?php echo e(__('crm.cancel')); ?>
            ...
            <!-- Edit WHM Credentials Modal -->
            <div id="editWhmCredentialsModal" class="fixed inset-0 z-50 hidden" onclick="this.classList.add('hidden')">
                <!-- Backdrop -->
                <div class="fixed inset-0 bg-gray-500/75 transition-opacity"></div>
                    ...
                    <div class="relative bg-white rounded-lg shadow-xl w-[500px] mx-4 p-5 z-10" onclick="event.stopPropagation()">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                ...
                                <?php echo e($isArabic ? 'بيانات الدخول لـ WHM' : 'WHM Login Credentials'); ?>

                            </h3>
                            <button type="button" onclick="document.getElementById('editWhmCredentialsModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                            </button>
                        </div>
                        <form action="<?php echo e(route('admin.services.update-whm-credentials', $service)); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('PUT'); ?>
                            <?php
                                $metadata = $service->metadata ?? [];
                                $existingLoginUrl = $metadata['whm_login_url'] ?? $metadata['login_url'] ?? '';
                                $existingUsername = $service->username ?? $metadata['whm_username'] ?? '';
                                $existingPassword = $service->decrypted_password ?? '';
                            ?>
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        <?php echo e($isArabic ? 'رابط الدخول WHM' : 'WHM Login URL'); ?> <span class="text-red-500">*</span>
                                    </label>
                                    <input type="url" name="whm_login_url" required
                                           value="<?php echo e(old('whm_login_url', $existingLoginUrl)); ?>"
                                           placeholder="https://server.example.com:2087"
                                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600">
                                    <p class="mt-1 text-xs text-gray-500"><?php echo e($isArabic ? 'مثال: https://server.example.com:2087' : 'Example: https://server.example.com:2087'); ?></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        <?php echo e($isArabic ? 'اسم المستخدم' : 'Username'); ?> <span class="text-red-500">*</span>
                                    </label>
                                    <input type="text" name="username" required
                                           value="<?php echo e($existingUsername); ?>"
                                           placeholder="reseller_user"
                                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        <?php echo e($isArabic ? 'كلمة المرور' : 'Password'); ?> <span class="text-red-500">*</span>
                                    </label>
                                    <div class="relative" x-data="{ showPassword: false }">
                                        <input :type="showPassword ? 'text' : 'password'" name="password" required
                                               value="<?php echo e($existingPassword); ?>"
                                               placeholder="••••••••"
                                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600 <?php echo e($isArabic ? 'pl-10' : 'pr-10'); ?>">
                                        <button type="button" @click="showPassword = !showPassword" 
                                                class="absolute <?php echo e($isArabic ? 'left-3' : 'right-3'); ?> top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                            <svg x-show="!showPassword" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                            </svg>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        <?php echo e($isArabic ? 'عنوان IP الحساب' : 'Account IP Address'); ?>

                                        <span class="text-xs font-normal text-gray-500"><?php echo e($isArabic ? '(اختياري)' : '(optional)'); ?></span>
                                    </label>
                                    <input type="text" name="whm_ip_address"
                                           value="<?php echo e(old('whm_ip_address', $metadata['whm_ip_address'] ?? '')); ?>"
                                           placeholder="192.168.1.1"
                                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600">
                                    <p class="mt-1 text-xs text-gray-500"><?php echo e($isArabic ? 'عنوان IP المخصص لحساب الريسلر' : 'The dedicated IP address for this reseller account'); ?></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        <?php echo e($isArabic ? 'ملاحظة للعميل' : 'Note for Customer'); ?>

                                        <span class="text-xs font-normal text-gray-500"><?php echo e($isArabic ? '(اختياري)' : '(optional)'); ?></span>
                                    </label>
                                    <textarea name="whm_customer_note" rows="3"
                                              placeholder="<?php echo e($isArabic ? 'أدخل أي ملاحظات أو تعليمات للعميل...' : 'Enter any notes or instructions for the customer...'); ?>"
                                              class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600 resize-none"><?php echo e(old('whm_customer_note', $metadata['whm_customer_note'] ?? '')); ?></textarea>
                                    <p class="mt-1 text-xs text-gray-500"><?php echo e($isArabic ? 'هذه الملاحظة ستظهر للعميل في صفحة الخدمة' : 'This note will be displayed to the customer on their service page'); ?></p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <input type="checkbox" name="activate_service" id="activate_service" value="1"
                                           class="w-4 h-4 text-slate-700 border-gray-300 rounded focus:ring-slate-600"
                                           <?php echo e($service->status === 'pending' ? 'checked' : ''); ?>>
                                    <label for="activate_service" class="text-sm text-gray-700">
                                        <?php echo e($isArabic ? 'تفعيل الخدمة بعد حفظ البيانات' : 'Activate service after saving credentials'); ?>

                                    </label>
                                </div>
                            </div>
                            <div class="bg-slate-50 border border-slate-200 rounded-lg p-3 mt-4 mb-4">
                                <p class="text-xs text-slate-800">
                                    <svg class="w-4 h-4 inline-block <?php echo e($isArabic ? 'ml-1' : 'mr-1'); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                    <?php echo e($isArabic ? 'هذه البيانات ستظهر للعميل في صفحة خدمته. تأكد من صحتها قبل الحفظ.' : 'These credentials will be visible to the client on their service page. Please verify them before saving.'); ?>

                                </p>
                            </div>
                            <div class="flex gap-3">
                                <button type="button" onclick="document.getElementById('editWhmCredentialsModal').classList.add('hidden')" 
                                        class="flex-1 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
                                    <?php echo e(__('crm.cancel')); ?>

                                </button>
                                <button type="submit" 
                                        class="flex-1 px-4 py-2 text-sm font-medium text-white bg-slate-700 rounded-lg hover:bg-slate-800 transition-colors">
                                    <?php echo e($isArabic ? 'حفظ البيانات' : 'Save Credentials'); ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('editWhmCredentialsModal').classList.remove('hidden');
    });
</script>
<?php endif; ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\resources\views/admin/services/reseller-show.blade.php ENDPATH**/ ?>