...
<?php $__env->startSection('title', __('frontend.my_cart') . ' - ' . config('app.name')); ?>
        ...
        <div class="bg-red-600 text-white px-5 py-3 rounded-lg shadow-lg flex items-center gap-3 min-w-[320px] max-w-md">
            ...
            <div class="flex-1">
                <p class="font-medium text-sm" id="dns-toast-message">
                    <?php echo e(__('frontend.dns_no_arabic') ?? 'Arabic characters are not allowed in DNS fields'); ?>

                </p>
                <p class="text-xs mt-0.5 opacity-80" id="dns-toast-subtitle">
                    <?php echo e(__('frontend.english_only_allowed') ?? 'Only English characters allowed'); ?>
            ...
            <div class="flex items-start gap-3 mb-3">
                ...
                <div class="flex-1">
                    <p class="font-medium text-sm">
                        <?php echo e(__('frontend.confirm_remove_item') ?? 'Are you sure you want to remove this item?'); ?>

                    </p>
                    <p class="text-xs mt-0.5 opacity-80">
                        <?php echo e(__('frontend.action_cannot_be_undone') ?? 'This action cannot be undone'); ?>

                    </p>
                </div>
            </div>
            <div class="flex gap-2 <?php echo e(app()->getLocale() == 'ar' ? 'flex-row-reverse' : ''); ?>">
                <button onclick="confirmRemove()" class="flex-1 bg-white/20 hover:bg-white/30 text-white font-medium py-2 px-4 rounded-lg transition-colors text-sm">
                    <?php echo e(__('frontend.yes_remove') ?? 'Yes, Remove'); ?>
            ...
            <div class="text-center mb-10">
                <div class="inline-flex items-center gap-2 px-4 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg mb-4">
                    ...
                    <span class="text-sm font-medium text-slate-700 dark:text-slate-300">
                        <?php echo e(__('frontend.shopping_cart') ?? 'Shopping Cart'); ?>
            ...
            <?php if(empty($cart)): ?>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-12 text-center border border-slate-200 dark:border-slate-700">
                    ...
                    <h2 class="text-2xl font-bold text-slate-900 dark:text-white mb-2">
                        <?php echo e(__('frontend.cart_is_empty')); ?>

                    </h2>
                    <p class="text-slate-600 dark:text-slate-400 mb-8 max-w-md mx-auto">
                        <?php echo e(__('frontend.start_shopping_message') ?? 'Start adding domains to your cart to build your online presence'); ?>

                    </p>
                    
                    <!-- CTA Buttons -->
                    <div class="flex flex-col sm:flex-row gap-3 justify-center">
                        <a href="<?php echo e(route('domains.search')); ?>" class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-slate-800 hover:bg-slate-900 dark:bg-slate-700 dark:hover:bg-slate-600 text-white font-medium rounded-lg transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                            </svg>
                            <span><?php echo e(__('frontend.search_domains') ?? 'Search Domains'); ?></span>
                        </a>
                        
                        <a href="<?php echo e(route('home')); ?>" class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-900 dark:text-white font-medium rounded-lg transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                            </svg>
                            <span><?php echo e(__('frontend.go_home') ?? 'Go Home'); ?></span>
                        </a>
                    </div>
                </div>
            <?php else: ?>
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        <?php $__currentLoopData = $cart; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php if(($item['type'] ?? 'domain') == 'hosting'): ?>
                                ...
                                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5 border border-slate-200 dark:border-slate-700" data-hosting-product="<?php echo e($item['product_id']); ?>">
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="flex-1">
                                            ...
                                            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-3" data-hosting-name>
                                                <?php echo e($item['product_name'] ?? __('frontend.hosting_plan')); ?>
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
                                                ...
                                                <div class="flex items-center gap-2 p-2.5 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-500 dark:text-slate-400"><?php echo e(__('frontend.domain')); ?></div>
                                                        <div class="text-sm font-medium text-slate-900 dark:text-white"><?php echo e($item['domain'] ?? '-'); ?></div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-2 p-2.5 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.billing_cycle')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white"><?php echo e(__('frontend.' . ($item['billing_cycle'] ?? 'monthly'))); ?></div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg mb-3">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.datacenter_location')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white">
                                                            <?php echo e($item['datacenter_name']); ?>

                                                            <?php if(isset($item['datacenter_price']) && $item['datacenter_price'] > 0): ?>
                                                                <span class="text-xs text-purple-600 dark:text-purple-400">(+$<?php echo e(number_format($item['datacenter_price'], 2)); ?>)</span>
                                                            <?php endif; ?>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <?php if(isset($item['cpanel_accounts']) && $item['cpanel_accounts'] > 0): ?>
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg mb-3">
                                                    <svg class="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                                    </svg>
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.cpanel_accounts')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white">
                                                            <?php echo e($item['cpanel_accounts']); ?> <?php echo e(__('frontend.accounts')); ?>

                                                            <?php if(isset($item['cpanel_tier_price']) && $item['cpanel_tier_price'] > 0): ?>
                                                                <span class="text-xs text-blue-600 dark:text-blue-400">(+$<?php echo e(number_format($item['cpanel_tier_price'], 2)); ?>)</span>
                                                            <?php endif; ?>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex flex-wrap gap-2 mb-3">
                                                    ...
                                                    <span class="inline-flex items-center gap-1 px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-medium rounded">
                                                        ...
                                                        <?php echo e(__('frontend.domain_privacy')); ?>
                                                ...
                                                <div class="text-<?php echo e(app()->getLocale() == 'ar' ? 'left' : 'right'); ?>">
                                                    <div class="mb-2">
                                                        ...
                                                        <div class="text-xs text-slate-500 dark:text-slate-400">
                                                            <?php echo e(__('frontend.hosting_price')); ?>

                                                        </div>
                                                    </div>
                                                    <?php if(isset($item['setup_fee']) && $item['setup_fee'] > 0): ?>
                                                    <div class="mb-2">
                                                        <div class="text-sm font-medium text-slate-600 dark:text-slate-300">
                                                            $<?php echo e(number_format($item['setup_fee'], 2)); ?>

                                                        </div>
                                                        <div class="text-xs text-slate-500 dark:text-slate-400">
                                                            <?php echo e(__('frontend.setup_fee')); ?>

                                                        </div>
                                                    </div>
                                                    <?php endif; ?>
                                                </div>
                                                <button onclick="removeFromCart('<?php echo e($key); ?>')" class="p-2 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg transition-colors">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="flex items-center gap-3 mb-3">
                                                ...
                                                <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-medium rounded-md">
                                                    ...
                                                    VPS <?php echo e(__('frontend.hosting')); ?>
                                            ...
                                            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-3">
                                                <?php echo e($item['product_name'] ?? __('frontend.vps_plan')); ?>
                                            ...
                                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                                                ...
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.cpu')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white"><?php echo e($item['vcpu_count'] ?? '-'); ?> <?php echo e(__('frontend.cores')); ?></div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.ram')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white">
                                                            <?php echo e(isset($item['ram_mb']) ? ($item['ram_mb'] >= 1024 ? ($item['ram_mb'] / 1024) . ' GB' : $item['ram_mb'] . ' MB') : '-'); ?>
                                                ...
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.storage')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white"><?php echo e($item['storage_gb'] ?? '-'); ?> GB SSD</div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.bandwidth')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white">
                                                            <?php echo e(isset($item['bandwidth_gb']) ? ($item['bandwidth_gb'] >= 1024 ? ($item['bandwidth_gb'] / 1024) . ' TB' : $item['bandwidth_gb'] . ' GB') : '-'); ?>
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                                ...
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.hostname')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white"><?php echo e($item['hostname']); ?></div>
                                                    </div>
                                                </div>
                                                <?php endif; ?>
                                                ...
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.public_ipv4')); ?></div>
                                                        <div class="text-xs text-slate-500 dark:text-slate-400"><?php echo e(__('frontend.included')); ?></div>
                                                    </div>
                                                </div>
                                                
                                                <!-- IPv6 Status -->
                                                <?php if(isset($item['enable_ipv6']) && $item['enable_ipv6']): ?>
                                                <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                    <svg class="w-5 h-5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                    </svg>
                                                    <div>
                                                        <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.public_ipv6')); ?></div>
                                                        <div class="text-xs text-slate-500 dark:text-slate-400"><?php echo e(__('frontend.free')); ?></div>
                                                    </div>
                                                </div>
                                                <?php endif; ?>
                                            </div>
                                        ...
                                        <div class="flex flex-col items-end gap-3">
                                            <div class="text-<?php echo e(app()->getLocale() == 'ar' ? 'left' : 'right'); ?>">
                                                <div class="mb-2">
                                                    ...
                                                    <div class="text-xs text-slate-500 dark:text-slate-400">
                                                        <?php echo e(__('frontend.vps_price')); ?>
                                                ...
                                                <?php endif; ?>
                                                <?php if(isset($item['enable_backups']) && $item['enable_backups'] && isset($item['backup_cost']) && $item['backup_cost'] > 0): ?>
                                                <div class="mb-2 text-xs text-slate-500">
                                                    <span class="font-medium">$<?php echo e(number_format($item['backup_cost'], 2)); ?></span> <?php echo e(__('frontend.automatic_backups')); ?>

                                                </div>
                                                <?php endif; ?>
                                                <?php if(isset($item['additional_ipv4']) && $item['additional_ipv4'] > 0 && isset($item['ipv4_cost']) && $item['ipv4_cost'] > 0): ?>
                                                <div class="mb-2 text-xs text-slate-500">
                                                    <span class="font-medium">$<?php echo e(number_format($item['ipv4_cost'], 2)); ?></span> <?php echo e($item['additional_ipv4']); ?> <?php echo e(__('frontend.additional_ips')); ?>

                                                </div>
                                                <?php endif; ?>
                                                <?php if(isset($item['enable_ddos']) && $item['enable_ddos'] && isset($item['ddos_cost']) && $item['ddos_cost'] > 0): ?>
                                                <div class="mb-2 text-xs text-slate-500">
                                                    <span class="font-medium">$<?php echo e(number_format($item['ddos_cost'], 2)); ?></span> <?php echo e(__('frontend.ddos_protection')); ?>
                                ...
                                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5 border border-slate-200 dark:border-slate-700">
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="flex-1">
                                            ...
                                            <!-- Server Name -->
                                            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-3">
                                                <?php echo e($item['product_name'] ?? __('frontend.dedicated_plan')); ?>
                                            ...
                                            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                                                ...
                                                <div class="flex items-center gap-2 p-2.5 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-500 dark:text-slate-400"><?php echo e(__('frontend.cpu')); ?></div>
                                                        <div class="text-sm font-medium text-slate-900 dark:text-white"><?php echo e($item['cpu_cores'] ?? '-'); ?> <?php echo e(__('frontend.cores')); ?></div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-2 p-2.5 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                                                    ...
                                                    <div>
                                                        <div class="text-xs text-slate-500 dark:text-slate-400"><?php echo e(__('frontend.ram')); ?></div>
                                                        <div class="text-sm font-bold text-slate-900 dark:text-white"><?php echo e($item['ram_gb'] ?? '-'); ?> GB</div>
                                                    </div>
                                                </div>
                                                ...
                                                <!-- Storage -->
                                                    <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                        ...
                                                        <div>
                                                            <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.storage')); ?></div>
                                                            <div class="text-sm font-bold text-slate-900 dark:text-white"><?php echo e($item['storage_total_gb'] ?? '-'); ?> GB</div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                        ...
                                                        <div>
                                                            <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.bandwidth')); ?></div>
                                                            <div class="text-sm font-bold text-slate-900 dark:text-white"><?php echo e($item['bandwidth'] ?? '-'); ?></div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                                    ...
                                                    <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                        ...
                                                        <div>
                                                            <div class="text-xs text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.public_ipv4')); ?></div>
                                                            <div class="text-xs text-slate-500 dark:text-slate-400">1 <?php echo e(__('frontend.included')); ?></div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="flex flex-col items-end gap-3">
                                                <div class="text-<?php echo e(app()->getLocale() == 'ar' ? 'left' : 'right'); ?>">
                                                    <div class="mb-2">
                                                        ...
                                                        <div class="text-xs text-slate-500 dark:text-slate-400">
                                                            <?php echo e(__('frontend.dedicated_price')); ?>
                            ...
                            <?php else: ?>
                                ...
                                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5 border border-slate-200 dark:border-slate-700" data-domain-key="<?php echo e($key); ?>">
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="flex-1">
                                            <!-- Domain Icon & Type -->
                                            <div class="flex items-center gap-3 mb-3">
                                                ...
                                                <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-medium rounded-md">
                                                    <?php if(isset($item['action']) && $item['action'] == 'register'): ?>
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                                        </svg>
                                                        <?php echo e(__('frontend.domain_registration')); ?>

                                                    <?php elseif(isset($item['action']) && $item['action'] == 'transfer'): ?>
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-3 break-all">
                                                <?php echo e($item['domain'] ?? $item['product_name'] ?? 'Domain'); ?>
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
                                                ...
                                                <div>
                                                    <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-2">
                                                        <?php echo e(__('frontend.registration_period') ?? 'Registration Period'); ?>

                                                    </label>
                                                    <div class="flex items-center gap-2">
                                                        <button onclick="updateYears('<?php echo e($key); ?>', -1)" class="w-8 h-8 flex items-center justify-center bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg transition-colors">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/>
                                                            </svg>
                                                        </button>
                                                        <div class="flex-1 text-center">
                                                            <span id="years-<?php echo e($key); ?>" class="text-lg font-bold text-slate-900 dark:text-white">
                                                                <?php echo e($item['years'] ?? 1); ?>

                                                            </span>
                                                            <span class="text-sm text-slate-600 dark:text-slate-400 ml-1">
                                                                <?php echo e(__('frontend.years') ?? 'Years'); ?>
                                                ...
                                                <div>
                                                    <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-2">
                                                        <?php echo e(__('frontend.privacy_protection') ?? 'Privacy Protection'); ?>
                                                    ...
                                                    <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                                                        <div class="flex items-center gap-2">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div class="text-xs text-slate-500 dark:text-slate-400">
                                                                    <?php echo e(__('frontend.free') ?? 'FREE'); ?>

                                                                </div>
                                                            </div>
                                                        </div>
                                                        <label class="relative inline-flex items-center cursor-pointer">
                                                            <input type="checkbox" id="privacy-<?php echo e($key); ?>" class="sr-only peer" <?php echo e(($item['privacy'] ?? true) ? 'checked' : ''); ?> onchange="togglePrivacy('<?php echo e($key); ?>')">
                                            ...
                                            <div class="mb-3">
                                                <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-2">
                                                    ...
                                                    <?php echo e(__('frontend.dns_settings') ?? 'DNS Settings'); ?>
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                    <label class="flex items-center p-3 bg-white dark:bg-slate-800 rounded-lg border-2 border-slate-200 dark:border-slate-700 cursor-pointer hover:border-blue-500 dark:hover:border-blue-500 transition-colors">
                                                        <input type="radio" name="dns-type-<?php echo e($key); ?>" value="default" class="w-4 h-4 text-blue-600" <?php echo e((!isset($item['dns_type']) || $item['dns_type'] == 'default') ? 'checked' : ''); ?> onchange="toggleDNSType('<?php echo e($key); ?>', 'default')">
                                                        <span class="ml-2 text-sm font-medium text-slate-900 dark:text-white">
                                                            <?php echo e(__('frontend.use_default_dns') ?? 'Default'); ?>

                                                        </span>
                                                    </label>
                                                    
                                                    <label class="flex items-center p-3 bg-white dark:bg-slate-800 rounded-lg border-2 border-slate-200 dark:border-slate-700 cursor-pointer hover:border-blue-500 dark:hover:border-blue-500 transition-colors">
                                                        <input type="radio" name="dns-type-<?php echo e($key); ?>" value="custom" class="w-4 h-4 text-blue-600" <?php echo e((isset($item['dns_type']) && $item['dns_type'] == 'custom') ? 'checked' : ''); ?> onchange="toggleDNSType('<?php echo e($key); ?>', 'custom')">
                                                        <span class="ml-2 text-sm font-medium text-slate-900 dark:text-white">
                                                            <?php echo e(__('frontend.use_custom_dns') ?? 'Custom'); ?>
                                            ...
                                            <div id="custom-dns-<?php echo e($key); ?>" class="mb-3 <?php echo e((!isset($item['dns_type']) || $item['dns_type'] == 'default') ? 'hidden' : ''); ?>">
                                                <div class="text-xs text-slate-600 dark:text-slate-400 mb-2">
                                                    <span class="text-red-500">*</span> <?php echo e(__('frontend.dns_required_note') ?? 'At least 2 DNS servers required'); ?>

                                                </div>
                                                
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                                    <input type="text" id="dns1-<?php echo e($key); ?>" placeholder="Primary DNS * (ns1.example.com)" value="<?php echo e($item['dns1'] ?? ''); ?>" class="w-full px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 required-dns" oninput="preventArabicInput(this)" onchange="updateDNS('<?php echo e($key); ?>', 1, this.value)" required>
                                                    
                                                    <input type="text" id="dns2-<?php echo e($key); ?>" placeholder="Secondary DNS * (ns2.example.com)" value="<?php echo e($item['dns2'] ?? ''); ?>" class="w-full px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 required-dns" oninput="preventArabicInput(this)" onchange="updateDNS('<?php echo e($key); ?>', 2, this.value)" required>
                                                    
                                                    <input type="text" id="dns3-<?php echo e($key); ?>" placeholder="Third DNS (ns3.example.com)" value="<?php echo e($item['dns3'] ?? ''); ?>" class="w-full px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" oninput="preventArabicInput(this)" onchange="updateDNS('<?php echo e($key); ?>', 3, this.value)">
                                                    
                                                    <input type="text" id="dns4-<?php echo e($key); ?>" placeholder="Fourth DNS (ns4.example.com)" value="<?php echo e($item['dns4'] ?? ''); ?>" class="w-full px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" oninput="preventArabicInput(this)" onchange="updateDNS('<?php echo e($key); ?>', 4, this.value)">
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex flex-col items-end gap-4">
                                            <div class="text-<?php echo e(app()->getLocale() == 'ar' ? 'left' : 'right'); ?>" data-price-section>
                                                <!-- Item Total Price (Dynamic) -->
                                                <?php if(isset($item['is_free_with_hosting']) && $item['is_free_with_hosting']): ?>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <div class="text-xs text-slate-500 dark:text-slate-400">
                                                        <?php echo e(__('frontend.with_hosting_plan')); ?>
                                                ...
                                                <div class="mb-3">
                                                    <div class="text-xl font-bold text-slate-900 dark:text-white" id="item-total-<?php echo e($key); ?>" data-registration-price="<?php echo e($item['price']); ?>" data-renewal-price="<?php echo e($item['renewal_price'] ?? $item['price']); ?>">
                                                        <?php
                                                            $years = $item['years'] ?? 1;
                                                    ...
                                                    <div class="text-xs text-slate-500 dark:text-slate-400">
                                                        <?php echo e(__('frontend.total_for') ?? 'Total for'); ?> <span id="years-label-<?php echo e($key); ?>"><?php echo e($item['years'] ?? 1); ?></span> <?php echo e(($item['years'] ?? 1) == 1 ? __('frontend.year') : __('frontend.years')); ?>
                                                ...
                                                <div class="mb-2">
                                                    ...
                                                    <div class="text-xs text-slate-500 dark:text-slate-400 font-medium">
                                                        <?php echo e(__('frontend.registration_price') ?? 'Registration Price'); ?>

                                                    </div>
                                                </div>
                                                
                                                <!-- Renewal Price -->
                                                <?php if(isset($item['renewal_price']) && $item['renewal_price'] != $item['price']): ?>
                                                <div class="pt-2 border-t border-slate-200 dark:border-slate-700">
                                                    <div class="flex items-center gap-2 mb-1">
                                                        ...
                                                        <span class="text-lg font-bold text-slate-700 dark:text-slate-300">
                                                            $<?php echo e(number_format($item['renewal_price'], 2)); ?>

                                                        </span>
                                                    </div>
                                                    <div class="text-xs text-slate-500 dark:text-slate-400">
                                                        <?php echo e(__('frontend.renewal_price') ?? 'Renewal Price'); ?>

                                                    </div>
                                                </div>
                                                <?php elseif(isset($item['renewal_price'])): ?>
                                                <div class="pt-2 border-t border-slate-200 dark:border-slate-700">
                                                    <div class="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                                        </svg>
                                                        <span class="font-medium"><?php echo e(__('frontend.same_renewal_price') ?? 'Same renewal price'); ?></span>
                                                    </div>
                                                </div>
                                                <?php endif; ?>
                                                <?php endif; ?>
                    ...
                    <div class="lg:col-span-1" id="order-summary-column">
                        <div id="order-summary" class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-5 border border-slate-200 dark:border-slate-700">
                                ...
                                <div class="flex items-center justify-between mb-3 p-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                                    <span class="text-xs font-medium text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.total_items') ?? 'Total Items'); ?></span>
                                    <span class="text-lg font-bold text-slate-900 dark:text-white" id="cart-items-count" data-cart-count><?php echo e(count($cart)); ?></span>
                                </div>

                                <!-- Price Breakdown -->
                                <div class="space-y-2 mb-3 pb-3 border-b border-slate-200 dark:border-slate-700">
                                    <div class="flex justify-between items-center text-sm">
                                        <span class="text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.subtotal') ?? 'Subtotal'); ?></span>
                                        <span class="font-medium text-slate-900 dark:text-white" data-cart-subtotal>$<?php echo e(number_format($subtotal ?? $total, 2)); ?></span>
                                    </div>
                                    <div class="flex justify-between items-center text-sm">
                                        <div class="flex items-center gap-1">
                                            <span class="text-slate-600 dark:text-slate-400"><?php echo e(__('frontend.tax') ?? 'Tax'); ?></span>
                                            <span class="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-1.5 py-0.5 rounded font-medium"><?php echo e(__('frontend.free') ?? 'FREE'); ?></span>
                                        </div>
                                ...
                                <div class="mb-4 pb-4 border-b border-slate-200 dark:border-slate-700">
                                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">
                                        <svg class="w-3.5 h-3.5 inline-block <?php echo e(app()->getLocale() == 'ar' ? 'ml-1' : 'mr-1'); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                                        </svg>
                                        <?php echo e(__('frontend.coupon_code') ?? 'Coupon Code'); ?>

                                    </label>
                                    <div class="flex gap-2 items-stretch">
                                        <input type="text" id="coupon-input" placeholder="<?php echo e(__('frontend.enter_coupon') ?? 'Enter code'); ?>" class="flex-1 min-w-0 px-3 py-2 text-sm bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-slate-400 focus:border-slate-400 transition-all" value="<?php echo e(session('coupon_code', '')); ?>" <?php echo e(session('coupon_code') ? 'disabled' : ''); ?>>
                                        <button onclick="applyCoupon()" id="apply-coupon-btn" class="flex-shrink-0 px-4 py-2 bg-slate-800 hover:bg-slate-900 dark:bg-slate-600 dark:hover:bg-slate-500 text-white text-sm font-medium rounded-lg transition-colors whitespace-nowrap">
                                            <?php echo e(__('frontend.apply') ?? 'Apply'); ?>

                                        </button>
                                    </div>
                                    
                                    <!-- Coupon Success Message -->
                                    <div id="coupon-success" class="<?php echo e(session('coupon_description') ? '' : 'hidden'); ?> mt-2 p-2 bg-green-50 dark:bg-green-900/20 rounded-lg">
                                        <div class="flex items-center gap-2 text-xs">
                                            <svg class="w-4 h-4 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                            <span class="text-green-700 dark:text-green-300 font-medium" id="coupon-success-text"><?php echo e(session('coupon_description')); ?></span>
                                    ...
                                    <!-- Discount Display -->
                                    <div id="discount-display" class="<?php echo e(($discount ?? 0) > 0 ? '' : 'hidden'); ?>">
                                        <div class="mt-2 flex justify-between items-center text-sm">
                                            <span class="text-green-600 dark:text-green-400 font-bold"><?php echo e(__('frontend.discount') ?? 'Discount'); ?></span>
                                            <span class="text-green-600 dark:text-green-400 font-bold" id="discount-amount">-$<?php echo e(number_format($discount ?? 0, 2)); ?></span>
                                        </div>
                                    </div>
                                </div>

                                <!-- Subtotal Before Discount (shown only when coupon applied) -->
                                <div id="subtotal-before-discount" class="<?php echo e(($discount ?? 0) > 0 ? '' : 'hidden'); ?> mb-3 pb-3 border-b border-slate-200 dark:border-slate-700">
                                    <div class="flex justify-between items-center">
                                        <span class="text-slate-600 dark:text-slate-400 text-sm font-medium"><?php echo e(__('frontend.subtotal_before_discount') ?? 'Subtotal Before Discount'); ?></span>
                                        <span class="text-slate-900 dark:text-white font-bold line-through decoration-2 decoration-red-500" data-cart-subtotal-display>$<?php echo e(number_format($subtotal ?? $total, 2)); ?></span>
                                    </div>
                                </div>
                                ...
                                <div class="mb-4 p-3 bg-slate-800 dark:bg-slate-700 rounded-xl">
                                    <div class="flex justify-between items-center">
                                        <span class="text-slate-300 text-xs font-medium"><?php echo e(__('frontend.total_to_pay') ?? 'Total to Pay'); ?></span>
                                        <span class="text-xl font-bold text-white" data-cart-total>$<?php echo e(number_format($total, 2)); ?></span>
                                    </div>
                                    <?php if(($discount ?? 0) > 0): ?>
                                    <div class="mt-2 pt-2 border-t border-slate-600">
                                        <div class="flex items-center justify-center gap-2 text-green-400 text-xs">
                                            ...
                                            <span class="font-medium"><?php echo e(__('frontend.you_save') ?? 'You Save'); ?>: $<?php echo e(number_format($discount, 2)); ?></span>
                                        </div>
                                    </div>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="space-y-2 mb-4">
                                    <button onclick="proceedToCheckout()" class="block w-full py-3 bg-slate-800 hover:bg-slate-900 dark:bg-slate-600 dark:hover:bg-slate-500 text-white text-center text-sm font-medium rounded-lg transition-colors">
                                        <?php echo e(__('frontend.proceed_to_checkout')); ?>

                                    </button>
                                    <a href="<?php echo e(route('domains.search')); ?>" class="block w-full py-3 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-900 dark:text-white text-center text-sm font-medium rounded-lg transition-colors">
                                        <?php echo e(__('frontend.continue_shopping')); ?>
                                ...
                                <div class="flex items-center justify-center gap-4 pt-3 border-t border-slate-200 dark:border-slate-700">
                                    <div class="flex items-center gap-1 text-slate-500 dark:text-slate-400">
                                        ...
                                        <span class="text-xs"><?php echo e(__('frontend.secure_checkout') ?? 'Secure'); ?></span>
                                    </div>
                                    <div class="flex items-center gap-1 text-slate-500 dark:text-slate-400">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                ...
                <div class="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] safe-area-bottom">
                    <div class="container mx-auto px-4 py-3">
                        <div class="flex items-center justify-between gap-4">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-2">
                                    <span class="text-xs text-slate-500 dark:text-slate-400"><?php echo e(__('frontend.total') ?? 'Total'); ?></span>
                                    <span class="text-xs text-slate-400">•</span>
                                    <span class="text-xs text-slate-500 dark:text-slate-400" data-mobile-cart-count><?php echo e(count($cart)); ?> <?php echo e(__('frontend.items') ?? 'items'); ?></span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <span class="text-lg font-bold text-slate-900 dark:text-white" data-mobile-cart-total>$<?php echo e(number_format($total, 2)); ?></span>
                                    <?php if(($discount ?? 0) > 0): ?>
                                    <span class="text-xs line-through text-slate-400">$<?php echo e(number_format($subtotal ?? $total, 2)); ?></span>
                                    <span class="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-1.5 py-0.5 rounded font-medium">-$<?php echo e(number_format($discount, 2)); ?></span>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <!-- Checkout Button -->
                            <button onclick="proceedToCheckout()" class="flex-shrink-0 px-6 py-3 bg-slate-800 hover:bg-slate-900 dark:bg-slate-600 dark:hover:bg-slate-500 text-white text-sm font-medium rounded-xl transition-colors flex items-center gap-2">
                                <span><?php echo e(__('frontend.checkout') ?? 'Checkout'); ?></span>
...
function showDNSToast(customMessage = null, customSubtitle = null) {
    ...
    if (customMessage) {
        ...
        messageElement.textContent = '<?php echo e(__("frontend.dns_no_arabic") ?? "Arabic characters are not allowed in DNS fields"); ?>';
    }
    ...
    if (customSubtitle !== null) {
        ...
        subtitleElement.textContent = '<?php echo e(__("frontend.english_only_allowed") ?? "Only English characters allowed"); ?>';
...
let cartData = <?php echo json_encode($cart, 15, 512) ?>;
    ...
    if (item && item.type === 'domain' && item.is_free_with_hosting && item.linked_hosting) {
        ...
        if (!hostingExists) {
            ...
            const message = hostingName 
                ? '<?php echo e(__('frontend.cannot_remove_linked_domain_with_name')); ?>'.replace(':hosting', hostingName)
                : '<?php echo e(__('frontend.cannot_remove_linked_domain')); ?>';
...
function confirmRemove() {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (card) {
                ...
                card.style.transform = '<?php echo e(app()->getLocale() == "ar" ? "translateX(-50px)" : "translateX(50px)"); ?>';
            ...
            showDNSToast(data.message || '<?php echo e(__("frontend.failed_to_remove_item") ?? "Failed to remove item"); ?>', '');
    ...
    .catch(error => {
        ...
        showDNSToast('<?php echo e(__("frontend.failed_to_remove_item") ?? "Failed to remove item. Please try again."); ?>', '');
        buttonToUpdate.disabled = false;
        buttonToUpdate.innerHTML = originalHTML;
    });
...
function updateYears(key, change) {
    ...
    fetch('<?php echo e(route("cart.update-years")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    .then(data => {
        if (data.success) {
            ...
            showDNSToast(data.message, '<?php echo e(__("frontend.error") ?? "Error"); ?>');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        ...
        showDNSToast('<?php echo e(__("frontend.error_occurred") ?? "An error occurred"); ?>', '<?php echo e(__("frontend.error") ?? "Error"); ?>');
    });
}
...
function togglePrivacy(key) {
    ...
    fetch('<?php echo e(route("cart.toggle-privacy")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
function restoreDomainPrice(domainKey, restoredPrice) {
    ...
    priceSection.innerHTML = `
        ...
        <div class="mb-3">
            ...
            <div class="text-xs text-slate-500 dark:text-slate-400">
                <?php echo e(__('frontend.total_for') ?? 'Total for'); ?> <span id="years-label-${domainKey}">${years}</span> ${years == 1 ? '<?php echo e(__('frontend.year')); ?>' : '<?php echo e(__('frontend.years')); ?>'}
            </div>
        </div>
...
function updateCartTotal(newTotal, newSubtotal = null, newDiscount = null, itemCount = null) {
    ...
    if (itemCount !== null) {
        ...
        if (mobileCartCount) {
            mobileCartCount.textContent = itemCount + ' <?php echo e(__("frontend.items") ?? "items"); ?>';
        }
    ...
    if (newDiscount !== null && newDiscount > 0) {
        ...
        youSaveElements.forEach(element => {
            if (element.textContent.includes('<?php echo e(__("frontend.you_save")); ?>') || element.textContent.includes('You Save') || element.textContent.includes('توفر')) {
                element.textContent = '<?php echo e(__("frontend.you_save") ?? "You Save"); ?>: $' + parseFloat(newDiscount).toFixed(2);
            }
        });
    }
}
...
function toggleDNSType(key, type) {
    ...
    fetch('<?php echo e(route("cart.update-dns-type")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
function updateDNS(key, dnsNumber, value) {
    ...
    if (arabicPattern.test(value)) {
        ...
        // Show alert
        alert('<?php echo e(__("frontend.dns_no_arabic") ?? "Arabic characters are not allowed in DNS fields"); ?>');
    ...
    fetch('<?php echo e(route("cart.update-dns")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
function applyCoupon() {
    ...
    if (!couponCode) {
        showCouponError('<?php echo e(__("frontend.enter_coupon_code") ?? "Please enter a coupon code"); ?>');
        return;
    }
    ...
    .catch(error => {
        ...
        applyBtn.disabled = false;
        applyBtn.innerHTML = originalBtnText;
        showCouponError('<?php echo e(__("frontend.error_occurred") ?? "An error occurred"); ?>');
    });
}

// Remove Coupon
function removeCoupon() {
    fetch('<?php echo e(route("cart.remove-coupon")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
function updatePricesWithDiscount(newTotal, discount, subtotal = null) {
    ...
    if (discount > 0) {
        ...
        if (grandTotal) {
            ...
            if (youSaveText) {
                youSaveText.textContent = '<?php echo e(__("frontend.you_save") ?? "You Save"); ?>: $' + parseFloat(discount).toFixed(2);
            }
        }
    } else {
...
function proceedToCheckout() {
    ...
    customDnsSections.forEach(section => {
        ...
        if (!dns1.value.trim() || !dns2.value.trim()) {
            hasError = true;
            errorMessage = '<?php echo e(__("frontend.dns_validation_error") ?? "Please enter at least 2 DNS servers for domains with custom DNS"); ?>';
...
<?php echo $__env->make('frontend.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\resources\views/frontend/cart/index.blade.php ENDPATH**/ ?>